...
public class ItemNatureGuardianSickle extends ArsMagicaItem {
        ...
        for (int i = -radius; i <= radius; ++i) {
            for (int j = -radius; j <= radius; ++j) {
                for (int k = -radius; k <= radius; ++k) {
                    ...
                    if (ExtendedProperties.For(par7EntityLivingBase)
                        .getCurrentMana() < 5f) {
                        if (par2World.isRemote) AMCore.proxy.flashManaBar();
                        return false;
                    }
                    ...
                    if (nextBlock instanceof BlockLeaves) {
                        if (ForgeEventFactory.doPlayerHarvestCheck(
                            DummyEntityPlayer.fromEntityLiving(par7EntityLivingBase),
                            nextBlock,
                            true)) if (!par2World.isRemote) par2World.func_147478_e(par4 + i, par5 + j, par6 + k, true);
                        ExtendedProperties.For(par7EntityLivingBase)
                            .deductMana(5f);
                    }
                }
            }
    ...
    public boolean flingSickle(ItemStack stack, World world, EntityPlayer player) {
        if (ExtendedProperties.For(player)
            .getCurrentMana() < 250 && !player.capabilities.isCreativeMode) {
