...
public class SpellScrollRenderer implements IItemRenderer {
    ...
    public boolean handleRenderType(ItemStack item, ItemRenderType type) {
        if (type == ItemRenderType.EQUIPPED || type == ItemRenderType.EQUIPPED_FIRST_PERSON) {
            if (Minecraft.getMinecraft().inGameHasFocus) {
                if (item.getItem() instanceof ItemSpellBase || (item.getItem() instanceof ItemSpellBook
                    && ((ItemSpellBook) item.getItem()).GetActiveScroll(item) != null)) {
                    return true;
                }
            }
        }
    ...
    private void renderFirstPersonArm(EntityClientPlayerMP player) {
        ...
        float f5 = this.mc.theWorld.getLightBrightness(
