...
public class ItemCrystalWrench extends ArsMagicaRotatedItem {
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        } else if (te != null && te instanceof TileEntityCrystalMarker
            && stack.stackTagCompound != null
            && stack.stackTagCompound.hasKey(HAB_PAIRLOC)) {
    ...
    private void handleCMPair(ItemStack stack, World world, EntityPlayer player, TileEntity te, double hitX,
        ...
        if (world.isRemote) {
            ...
            if (!stack.stackTagCompound.hasKey(KEEP_BINDING)) stack.stackTagCompound.removeTag(HAB_PAIRLOC);
        }
    }
    ...
    private void doPairNodes(World world, int x, int y, int z, ItemStack stack, EntityPlayer player, double hitX,
        double hitY, double hitZ, TileEntity te) {
        ...
        if (!stack.stackTagCompound.hasKey(KEEP_BINDING)) stack.stackTagCompound.removeTag(KEY_PAIRLOC);
    }

    private void doDisconnect(IPowerNode node, World world, double hitX, double hitY, double hitZ,
        EntityPlayer player) {
        PowerNodeRegistry.For(world)
            .tryDisconnectAllNodes(node);
        if (world.isRemote) {
    ...
    private void handleModeChanges(ItemStack stack) {
        if (stack.stackTagCompound.hasKey(KEEP_BINDING)) {
            ...
            if (stack.stackTagCompound.hasKey(KEY_PAIRLOC)) stack.stackTagCompound.removeTag(KEY_PAIRLOC);

            if (stack.stackTagCompound.hasKey(HAB_PAIRLOC)) stack.stackTagCompound.removeTag(HAB_PAIRLOC);
        } else {
            if (getMode(stack) == MODE_PAIR) stack.stackTagCompound.setInteger(MODE, MODE_DISCONNECT);
            else stack.stackTagCompound.setInteger(MODE, MODE_PAIR);
    ...
    private IIcon GetWrenchIcon(ItemStack stack, int pass) {
        if (stack.stackTagCompound != null && pass == 0) {
            if (stack.stackTagCompound.hasKey(KEEP_BINDING)) return wrenchStoredIcon;
