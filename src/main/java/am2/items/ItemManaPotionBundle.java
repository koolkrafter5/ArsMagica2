...
public class ItemManaPotionBundle extends ArsMagicaItem {
    ...
    private IIcon[] icons;
    private final String[] textureFiles = { "potion_bundle_lesser", "potion_bundle_standard", "potion_bundle_greater",
        "potion_bundle_epic", "potion_bundle_legendary" };
    ...
    public ItemStack onEaten(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
        ...
        par1ItemStack
            .setItemDamage(((par1ItemStack.getItemDamage() >> 8) << 8) + getUses(par1ItemStack.getItemDamage()) - 1);
    ...
    private void giveOrDropItem(EntityPlayer player, ItemStack stack) {
        if (!player.inventory.addItemStackToInventory(stack)) player.dropPlayerItemWithRandomChoice(stack, true);
    }
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        par3List.add(
            "" + getUses(par1ItemStack.getItemDamage())
                + " "
                + StatCollector.translateToLocal("am2.tooltip.uses")
                + ".");
    }
    ...
    public String getItemStackDisplayName(ItemStack par1ItemStack) {
        ...
        if (potion == ItemsCommonProxy.lesserManaPotion) {
            return String.format(
                "%s %s",
                StatCollector.translateToLocal("item.arsmagica2:mana_potion_lesser.name"),
                StatCollector.translateToLocal("item.arsmagica2:potionBundle.name"));
        } else if (potion == ItemsCommonProxy.standardManaPotion) {
            return String.format(
                "%s %s",
                StatCollector.translateToLocal("item.arsmagica2:mana_potion_standard.name"),
                StatCollector.translateToLocal("item.arsmagica2:potionBundle.name"));
        } else if (potion == ItemsCommonProxy.greaterManaPotion) {
            return String.format(
                "%s %s",
                StatCollector.translateToLocal("item.arsmagica2:mana_potion_greater.name"),
                StatCollector.translateToLocal("item.arsmagica2:potionBundle.name"));
        } else if (potion == ItemsCommonProxy.epicManaPotion) {
            return String.format(
                "%s %s",
                StatCollector.translateToLocal("item.arsmagica2:mana_potion_epic.name"),
                StatCollector.translateToLocal("item.arsmagica2:potionBundle.name"));
        } else if (potion == ItemsCommonProxy.legendaryManaPotion) {
            return String.format(
                "%s %s",
                StatCollector.translateToLocal("item.arsmagica2:mana_potion_legendary.name"),
                StatCollector.translateToLocal("item.arsmagica2:potionBundle.name"));
        }
