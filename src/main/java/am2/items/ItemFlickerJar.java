...
public class ItemFlickerJar extends ArsMagicaItem {
    ...
    public IIcon getIcon(ItemStack stack, int pass) {
        if (stack.getItemDamage() == 0) return itemIcon;

        if (pass == 0) return flickerIcon;
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        ...
        if (meta == 0) return String.format(
            StatCollector.translateToLocal("item.arsmagica2:flickerJar.name"),
            StatCollector.translateToLocal("am2.tooltip.empty"));

        Affinity aff = Affinity.values()[meta];
        baseName = String
            .format(StatCollector.translateToLocal("item.arsmagica2:flickerJar.name"), toProperCase(aff.name()));
