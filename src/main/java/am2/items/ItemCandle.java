...
public class ItemCandle extends ArsMagicaItem {
            ...
            if (player.isSneaking() && block != null
                ...
                if (!world.isRemote) {
                    ...
                    AMParticle particle = (AMParticle) AMCore.proxy.particleManager
                        .spawn(world, "radiant", x + 0.5, y + 0.5, z + 0.5);
                    if (particle != null) {
        ...
        if (!world.isRemote) {
            ...
            if (stack.hasTagCompound() && stack.stackTagCompound.hasKey("search_block")) {
                player.addChatMessage(
                    new ChatComponentText(StatCollector.translateToLocal("am2.tooltip.candlecantplace")));
                return false;
            }
    ...
    public void search(EntityPlayer player, ItemStack stack, World world, int cx, int cy, int cz, Block block,
        int meta) {
            ...
            for (int j = -1; j <= 1; ++j) {
                for (int k = -radius; k <= radius; ++k) {
                    ...
                    if (block == f_block && (meta == Short.MAX_VALUE || meta == f_meta)) {
                        if (Math.abs(i) <= immediate_radius && Math.abs(k) <= immediate_radius
                            && player.getCurrentArmor(3) != null
                            && ArmorHelper.isInfusionPreset(player.getCurrentArmor(3), GenericImbuement.pinpointOres)) {
        ...
        if (!found) setFlameColor(stack, 0, 1, 0);
    }
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        ...
        if (stack.hasTagCompound() && stack.stackTagCompound.hasKey("search_block")) {
            ...
            if (tempItem == null) {
                name += " (" + stack.stackTagCompound.getInteger("search_block")
                    + ":"
                    + stack.stackTagCompound.getInteger("search_meta")
                    + ")";
            } else {
