...
public class ModelArcaneGuardian extends ModelBase {
    ...
    public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
        ...
        if (entity instanceof EntityArcaneGuardian) {
            ...
            GL11.glPushMatrix(); // arm was rendering incorrectly previously. This is still a bit off, but it looks a
                                 // bit better now
        ...
        switch (guardian.getCurrentAction()) {
            case CASTING:
                ...
                if (guardian.getTicksInCurrentAction() < action_ticks) {
                    ...
                    main_arm_rotation_x = (float) Math.toRadians(
                        -max_degrees_x + (final_degrees_x
                            * ((guardian.getTicksInCurrentAction() + (f2 - guardian.ticksExisted) - action_ticks)
                                / fast_action_ticks)));
                    pants_rotation = (float) Math.toRadians(
                        -max_pants_degrees
                            * ((guardian.getTicksInCurrentAction() + (f2 - guardian.ticksExisted) - action_ticks)
                                / fast_action_ticks));
                } else {
                    ...
                    if (guardian.getTicksInCurrentAction() < action_ticks + fast_action_ticks + final_action_ticks)
                        pants_rotation = (float) Math.toRadians(
                            -max_pants_degrees + (max_pants_degrees
                                * ((guardian.getTicksInCurrentAction() + (f2 - guardian.ticksExisted)
                                    - action_ticks
                                    - fast_action_ticks) / final_action_ticks)));
                }
                break;
        }
