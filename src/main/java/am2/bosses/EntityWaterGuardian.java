...
public class EntityWaterGuardian extends AM2Boss {
    ...
    protected void initSpecificAI() {
        ...
        this.tasks
            .addTask(4, new EntityAICastSpell(this, NPCSpells.instance.waterBolt, 12, 23, 5, BossActions.CASTING));
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote && uberSpinAvailable
            && currentAction != BossActions.CASTING
            && currentAction != BossActions.IDLE) {
    ...
    private void updateRotations() {
        if (!isClone()) orbitRotation += 2f;
        else orbitRotation -= 2f;
        orbitRotation %= 360;
    ...
    public boolean attackEntityFrom(DamageSource par1DamageSource, float par2) {
        if (par1DamageSource.getSourceOfDamage() instanceof EntityWaterGuardian
            || par1DamageSource == DamageSource.drown) return false;
    ...
    protected float modifyDamageAmount(DamageSource source, float damageAmt) {
        if (source instanceof DamageSourceLightning) damageAmt *= 2.0f;
