...
public abstract class AM2Boss extends EntityMob implements IArsMagicaBoss, IEntityMultiPart {
    ...
    public boolean attackEntityFrom(DamageSource par1DamageSource, float par2) {
        ...
        if (par1DamageSource.isUnblockable() && !par1DamageSource.isMagicDamage()
            && !par1DamageSource.isDamageAbsolute()
            && !par1DamageSource.canHarmInCreative()) {
            ReflectionHelper
                .setPrivateValue(DamageSource.class, par1DamageSource, false, "isUnblockable", "field_76374_o");
        } // anti-TiC-rapier

        if (par1DamageSource == DamageSource.inWall) {
            if (!worldObj.isRemote) {// dead code? (calling canSnowAt() without using the result) could it be a buggy
                                     // upgrade to 1.7.10?
                for (int i = -1; i <= 1; ++i) {
                    for (int j = 0; j < 3; ++j) {
                        for (int k = -1; k <= 1; ++k) {
                    ...
                    if (!this.worldObj.isAirBlock((int) this.posX + x, (int) this.posY + y, (int) this.posZ + z)) {
                        if (this.worldObj.rand.nextDouble() > 0.993D
                            && block.getBlockHardness(
                                ...
                                (int) this.posX + x,
                                (int) this.posY + y,
                                (int) this.posZ + z) > 0.1f
                            && !(block instanceof BlockLiquid)
                            && !(disallowedBlocks.contains(block))
                            ...
                            block.breakBlock(
                                ...
                                block,
                                this.worldObj
                                    .getBlockMetadata((int) this.posX + x, (int) this.posY + y, (int) this.posZ + z));
