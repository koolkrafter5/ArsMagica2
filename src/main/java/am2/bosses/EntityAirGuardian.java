...
public class EntityAirGuardian extends AM2Boss {
    ...
    public void setCurrentAction(BossActions action) {
        ...
        if (action == BossActions.CASTING) this.useLeftArm = !this.useLeftArm;
    ...
    public void onUpdate() {
                ...
                if (this.worldObj.isRemote) {
                    for (int i = 0; i < AMCore.config.getGFXLevel(); ++i) {
                        AMParticle particle = (AMParticle) AMCore.proxy.particleManager.spawn(
                            worldObj,
                            "wind",
                            posX + worldObj.rand.nextDouble() * 4 - 2,
                            posY,
                            posZ + worldObj.rand.nextDouble() * 4 - 2);
                        if (particle != null) {
        ...
        if (this.posY < 150) {
            if (worldObj.isRemote) {
                for (int i = 0; i < 25; ++i) {
                    AMParticle wind = (AMParticle) AMCore.proxy.particleManager
                        .spawn(worldObj, "wind", posX, posY, posZ);
                    if (wind != null) {
                        wind.setIgnoreMaxAge(false);
                        wind.setMaxAge(10 + rand.nextInt(10));
                        wind.setDontRequireControllers();
                        wind.addRandomOffset(1, 1, 1);
                        // wind.AddParticleController(new PaticleFleePoint(wind, new AMVector3(this), rand.nextInt() *
                        // 0.1f + 0.01f, 10, 1, false));
                    }
                }
            } else {
                if (this.posY < 145) this.setDead();
            }
        }
    ...
    protected float modifyDamageAmount(DamageSource source, float damageAmt) {
        if (source.isMagicDamage()) damageAmt /= 2;
        else if (source instanceof DamageSourceLightning) damageAmt *= 2;
        return damageAmt;
    }
