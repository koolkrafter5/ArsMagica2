...
public class EntityAIStrikeAttack extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (cooldownTicks-- > 0 || ((IArsMagicaBoss) host).getCurrentAction() != BossActions.IDLE
            || !((IArsMagicaBoss) host).isActionValid(BossActions.STRIKE)) return false;
    ...
    public boolean continueExecuting() {
        ...
        if (AITarget == null || AITarget.isDead
            || (((IArsMagicaBoss) host).getCurrentAction() == BossActions.STRIKE
    ...
    public void updateTask() {
        ...
        if (((IArsMagicaBoss) host).getCurrentAction() == BossActions.STRIKE
            ...
            List<EntityLivingBase> aoeEntities = host.worldObj.getEntitiesWithinAABB(
                EntityLivingBase.class,
                host.boundingBox.getOffsetBoundingBox(offsetX, 0, offsetZ)
                    .expand(2.5, 2, 2.5));
