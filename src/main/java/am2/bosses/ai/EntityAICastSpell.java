...
public class EntityAICastSpell extends EntityAIBase {
    ...
    public EntityAICastSpell(IArsMagicaBoss host, ItemStack spell, int castPoint, int duration, int cooldown,
    ...
    public EntityAICastSpell(IArsMagicaBoss host, ItemStack spell, int castPoint, int duration, int cooldown,
        BossActions activeAction, ISpellCastCallback callback) {
        this.host = (EntityLiving) host;
        this.stack = spell;
        this.castPoint = castPoint;
        this.duration = duration;
    ...
    public boolean shouldExecute() {
        ...
        boolean execute = ((IArsMagicaBoss) host).getCurrentAction() == BossActions.IDLE
            ...
            && cooldownTicks <= 0;
        if (execute) {
            if (callback == null || callback.shouldCast(host, stack)) hasCasted = false;
            else execute = false;
        }
        return execute;
    }
    ...
    public void updateTask() {
        ...
        if (host.getDistanceSqToEntity(host.getAttackTarget()) > 64) {
            ...
            host.getNavigator()
                .tryMoveToXYZ(newX, host.getAttackTarget().posY, newZ, 0.5f);
        } else if (!host.canEntityBeSeen(host.getAttackTarget())) {
            host.getNavigator()
                .tryMoveToEntityLiving(host.getAttackTarget(), 0.5f);
        } else {
