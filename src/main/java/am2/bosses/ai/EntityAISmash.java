...
public class EntityAISmash extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (cooldownTicks-- > 0 || ((IArsMagicaBoss) host).getCurrentAction() != BossActions.IDLE
            || !((IArsMagicaBoss) host).isActionValid(BossActions.SMASH)) return false;
        ...
        if (AITarget != null && host.getDistanceSqToEntity(AITarget) > 4D) {
            if (!host.getNavigator()
                .tryMoveToEntityLiving(AITarget, moveSpeed)) return false;
        }
    ...
    public boolean continueExecuting() {
        EntityLivingBase AITarget = host.getAttackTarget();
        if (AITarget != null && host.getDistanceSqToEntity(AITarget) > 4D) {
            if (host.onGround) return host.getNavigator()
                .tryMoveToEntityLiving(AITarget, moveSpeed);
        }
        if (AITarget == null || AITarget.isDead
            || (((IArsMagicaBoss) host).getCurrentAction() == BossActions.SMASH
                && ((IArsMagicaBoss) host).getTicksInCurrentAction() > ((IArsMagicaBoss) host).getCurrentAction()
                    .getMaxActionTime())) {
    ...
    public void updateTask() {
        host.getLookHelper()
            .setLookPositionWithEntity(host.getAttackTarget(), 30, 30);
        host.getNavigator()
        ...
        if (((IArsMagicaBoss) host).getCurrentAction() == BossActions.SMASH
            ...
            List<EntityLivingBase> aoeEntities = host.worldObj
                .getEntitiesWithinAABB(EntityLivingBase.class, host.boundingBox.expand(4, 2, 4));
