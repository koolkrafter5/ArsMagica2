...
public class EntityAIHurricane extends EntityAIBase {
    ...
    public boolean continueExecuting() {
        ...
        if (AITarget == null || AITarget.isDead
            ...
            if (!host.worldObj.isRemote) {
                ...
                for (int x = -1; x <= 1; ++x) {
                    for (int z = -1; z <= 1; ++z) {
                        while (!host.worldObj.canBlockSeeTheSky((int) host.posX + x, y, (int) host.posZ + z)
                            && host.worldObj.getBlock((int) host.posX + x, y, (int) host.posZ + z) != Blocks.bedrock) {
            ...
            for (EntityLivingBase ent : nearbyEntities) {
                ...
                AMVector3 movement = MathUtilities
                    .GetMovementVectorBetweenPoints(new AMVector3(host), new AMVector3(ent));
                float factor = 2.15f;
    ...
    public void updateTask() {
        ...
        List<EntityLivingBase> nearbyEntities = host.worldObj
            .getEntitiesWithinAABB(EntityLivingBase.class, host.boundingBox.expand(6, 3, 6));
        for (EntityLivingBase ent : nearbyEntities) {
            if (ent == host) continue;
            ...
            AMVector3 movement = MathUtilities.GetMovementVectorBetweenPoints(
                new AMVector3(host).add(new AMVector3(0, host.getEyeHeight(), 0)),
                new AMVector3(ent));
            float factor = 0.18f;
