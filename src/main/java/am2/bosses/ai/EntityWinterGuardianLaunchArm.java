...
public class EntityWinterGuardianLaunchArm extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        if (host.getAttackTarget()
            .getDistanceSqToEntity(host) > 16D) {
            target = host.getAttackTarget();
            return true;
        }
        List<EntityLivingBase> entities = host.worldObj
            .getEntitiesWithinAABB(EntityLivingBase.class, host.boundingBox.expand(20, 20, 20));
        if (entities.size() > 0) {
    ...
    public boolean continueExecuting() {
        if (target == null || target.isDead
            || host.getDistanceSqToEntity(target) < 49D
            || host.getDistanceSqToEntity(target) > 225D
            || (host.getCurrentAction() == BossActions.LAUNCHING
                && host.getTicksInCurrentAction() > host.getCurrentAction()
                    .getMaxActionTime())) {
            host.setCurrentAction(BossActions.IDLE);
            target = null;
            if (!host.hasLeftArm() && !host.hasRightArm()) cooldownTicks = 20;
            return false;
        }
        return true;
    }
    ...
    public void updateTask() {
        ...
        if (host.getDistanceSqToEntity(target) > 144 && host.getCurrentAction() == BossActions.IDLE) {
            ...
            host.getNavigator()
                .clearPathEntity();
            if (host.getCurrentAction() != BossActions.LAUNCHING) host.setCurrentAction(BossActions.LAUNCHING);
