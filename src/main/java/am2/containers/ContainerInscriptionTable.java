...
public class ContainerInscriptionTable extends Container {
    ...
    public boolean slotIsBook(int slot) {
        return slotHasStack(slot) && ((Slot) this.inventorySlots.get(slot)).getStack()
            .getItem() == Items.written_book
            && ((Slot) this.inventorySlots.get(slot)).getStack()
                .getTagCompound() != null
            && !((Slot) this.inventorySlots.get(slot)).getStack()
                .getTagCompound()
    ...
    public boolean modifierCanBeAdded(ISpellModifier modifier) {
        ...
        for (SpellModifiers mod : modifiers) {
            if (table.getModifierCount(mod) > 2) return false;
        }
        return true;
    }
    ...
    public void resetSpellNameAndIcon() {
        ...
        if (stack != null) table.resetSpellNameAndIcon(stack, inventoryPlayer.player);
