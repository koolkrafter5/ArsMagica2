...
public class ContainerMagiciansWorkbench extends AM2Container {
    ...
    public ContainerMagiciansWorkbench(InventoryPlayer playerInventory, TileEntityMagiciansWorkbench tileEntity) {
        ...
        if (tileEntity.getUpgradeStatus(tileEntity.UPG_CRAFT)) INVENTORY_STORAGE_START += 5;
        ...
        secondCraftMatrix = tileEntity.getUpgradeStatus(tileEntity.UPG_CRAFT) ? new InventoryCrafting(this, 3, 3)
        ...
        // first output
        addSlotToContainer(
            new SlotMagiciansWorkbenchCrafting(
                playerInventory.player,
                firstCraftMatrix,
                tileEntity.firstCraftResult,
                this,
                0,
                37,
                89));
    ...
    public void onCraftMatrixChanged(IInventory par1iInventory) {
        this.workbenchInventory.firstCraftResult.setInventorySlotContents(
            0,
        ...
        this.workbenchInventory.secondCraftResult.setInventorySlotContents(
            0,
            CraftingManager.getInstance()
    ...
    public boolean gridIsFreeFor(int recipeIndex) {
        ...
        if (recipe.components.length > 4
            && !this.workbenchInventory.getUpgradeStatus(this.workbenchInventory.UPG_CRAFT)) {
    ...
    public boolean isRecipeAlreadyInGrid(int recipeIndex) {
        ...
        if (getWorkbench().firstCraftResult.getStackInSlot(0) != null
            && getWorkbench().firstCraftResult.getStackInSlot(0)
                .isItemEqual(recipe.output))
            return true;
        if (getWorkbench().secondCraftResult.getStackInSlot(0) != null
            && getWorkbench().secondCraftResult.getStackInSlot(0)
                .isItemEqual(recipe.output))
            return true;
        return false;
    }
