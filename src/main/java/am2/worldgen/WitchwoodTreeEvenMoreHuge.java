...
public class WitchwoodTreeEvenMoreHuge extends WorldGenAbstractTree {
    ...
    protected void setBlockAndMetadataWithNotify(int x, int y, int z, Block block, int meta) {
        try {
            ...
            // FMLLog.getLogger().error(new FormattedMessage("setBlockAndMetadataWithNotify(%s, %s, %s)", new Object[]{
            // x, y, z }));
        }
    }
    ...
    private boolean generateLeafNodeList() {
        ...
        if (basePos[1] + heightLimit > 256 - 4) {
            ...
            if (basePos[1] + heightLimit > 256 - 4) return false;
        }
    ...
    void placeBlockLine(int ai[], int ai1[]) {
        if (Math.abs(Math.abs(ai[0]) - Math.abs(ai1[0])) > 10 || Math.abs(Math.abs(ai[2]) - Math.abs(ai1[2])) > 10) {
            return; // anti tree bug
        }
    ...
    void growTapRoot(int i, int j, int k, double flen) {
        ...
        for (int jj = 1; jj <= len; jj++) {
            ...
            // if(zz != 0 && zz != trunkBlock && zz != trunkMeta && zz != 2 && zz != 3 && zz != 8 && zz != 9 && zz != 12
            // && zz != 13) break;
            // else
                ...
                if (med != 0) { // Grow normal.
                    ...
                    if (med != 0) { // Grow down.
                        ...
                        if (med != 0) { // Grow out.
                            ...
                            int dir = ((int) (direction * 8.0 / Math.PI)); // Integer direction - 16 = complete
                                                                           // rotation.
    ...
    boolean validTreeLocation() {
        ...
        boolean isValidSoil = i != Blocks.air && i.canSustainPlant(
            this.worldObject,
            this.basePos[0],
            this.basePos[1] - 1,
            this.basePos[2],
