...
public class AMEventHandler {
    ...
    public void onEntityConstructing(EntityConstructing event) {
        if (event.entity instanceof EntityLivingBase) {
            ...
            ((EntityLivingBase) event.entity).getAttributeMap()
        ...
        if (soonToBeDead instanceof EntityPlayer) { // soul fragments: die with at least 5 rare items
            if (soonToBeDead.isPotionActive(BuffList.psychedelic)) {
                if (soonToBeDead.worldObj.provider.dimensionId == 1
                    && soonToBeDead.getActivePotionEffect(BuffList.psychedelic)
                        .getAmplifier() == 1) {
                    ...
                    for (ItemStack stack : player.inventory.armorInventory) {
                        if (stack != null) {
                            if (stack.getRarity() != EnumRarity.common) {
                                player.inventory
                                    .setInventorySlotContents(slotCount + player.inventory.mainInventory.length, null);
                                rareCount++;
                            }
                        }
                        slotCount++;
                    ...
                    if (rareCount >= 5) {
                        ...
                        fragment.setPosition(
                            player.posX + rand.nextInt(5) - 2,
                            player.posY + 10,
                            player.posZ + rand.nextInt(5) - 2);
    ...
    public void onEntityLiving(LivingUpdateEvent event) {
        ...
        if (extendedProperties.getIsFlipped()) {
            ...
            if (world.rayTraceBlocks(
                ...
                if (event.entityLiving instanceof EntityPlayer) {
                    if (ent.motionY > 0) {
                        if (world.isRemote) ent.fallDistance += ent.posY - ent.prevPosY;
                        else ent.fallDistance += (((EntityPlayer) ent).field_71095_bQ
    ...
    private void HandleSpellEffects(LivingUpdateEvent event, ExtendedProperties extendedProperties) {
        ...
        if (ent.isPotionActive(BuffList.agility.id)) {
            if (ent.stepHeight != 1.01f) ent.stepHeight = 1.01f;
    ...
    private void HandlePlayerSpellExtendedProperties(EntityPlayer player, ExtendedProperties extendedProperties) {
        if (extendedProperties.hasExtraVariable("ethereal")) { // ethereal form handling
            ...
            if (ItemSoulspike.bootsHaveEtherealTag(player.inventory.armorInventory[0])) { // but boots have tag
                ItemSoulspike.removeTagFromBoots(player.inventory.armorInventory[0]); // remove tag; prevent possible
                                                                                      // workaround
            }
        }
        ...
        if (enabled_accelerate) {
            ...
            try {
                for (int l = 0; l < s1; l++) { // do this outside of for loop to save performance
                    ...
                    for (int f = 0; f < s2; f++) {
                        if (f >= worlds[l].loadedEntityList.size()) break; // fix for the most obscene bug ever, where
                                                                           // it doesn't respect indexes, or arbitrarily
                                                                           // chooses to delete entities while I'm
                                                                           // iterating over them
                        Object entityobj = worlds[l].loadedEntityList.get(f);
                        if (entityobj instanceof EntityLivingBase) {
                            String UUID = ((EntityLivingBase) entityobj).getUniqueID()
                                .toString();
        ...
        if (enabled_accelerate || enabled_slow || enable_spatialVortex || enabled_timeFortified || enabled_shield) {
            ...
            if ((tick % ((player.worldObj.playerEntities.size() + 1) * 5)) == 0) { // only does behavior every 5 player
                                ...
                                for (int zadd = -1; zadd <= 1; zadd += 2) {
                                    ...
                                    if (te != null) {
                                        ...
                                        if (te instanceof IPowerNode) {
                                            totaletheriumdark[vIndex] += PowerNodeRegistry.For(thisdim)
                                                .getPower((IPowerNode) te, PowerTypes.DARK);
                                            totaletheriumlight[vIndex] += PowerNodeRegistry.For(thisdim)
                                ...
                                if (localIndex == maxD[0]) {
                                    ...
                                    for (int xadd = -1; xadd <= 1; xadd += 2) {
                                        for (int zadd = -1; zadd <= 1; zadd += 2) {
                                            ...
                                            if (te != null) {
                                                if (te instanceof IPowerNode) {
                                                    if (toSubtractD > 0) toSubtractD -= PowerNodeRegistry.For(thisdim)
                                ...
                                if (localIndex == maxL[0]) {
                                    ...
                                    for (int xadd = -1; xadd <= 1; xadd += 2) {
                                        for (int zadd = -1; zadd <= 1; zadd += 2) {
                                            ...
                                            if (te != null) {
                                                if (te instanceof IPowerNode) {
                                                    if (toSubtractL > 0) toSubtractL -= PowerNodeRegistry.For(thisdim)
                                ...
                                if (fIndex == minD[0] && DLeft) {
                                    ...
                                    for (int xadd = -1; xadd <= 1; xadd += 2) {
                                        for (int zadd = -1; zadd <= 1; zadd += 2) {
                                            ...
                                            if (te != null) {
                                                if (te instanceof IPowerNode) {
                                                    if (toTransferD > 0) toTransferD -= PowerNodeRegistry.For(thisdim)
                                ...
                                if (fIndex == minL[0] && LLeft) {
                                    ...
                                    for (int xadd = -1; xadd <= 1; xadd += 2) {
                                        for (int zadd = -1; zadd <= 1; zadd += 2) {
                                            ...
                                            if (te != null) {
                                                if (te instanceof IPowerNode) {
                                                    if (toTransferL > 0) toTransferL -= PowerNodeRegistry.For(thisdim)
        ...
        if (player.isPotionActive(BuffList.flight.id) || player.isPotionActive(BuffList.levitation.id)
    ...
    private void handlePsychedelics(EntityPlayer player) {
        if (player.isPotionActive(BuffList.psychedelic)) {
            if (player.worldObj.provider.dimensionId == -1) { // any amplifier can cause lower level hallucinations
                if (player.getActivePotionEffect(BuffList.psychedelic)
                    .getDuration() > 10) lowerHallucinationTick(player);
                else cleanupLowerHallucination(player);
            } else if (player.worldObj.provider.dimensionId == 1 && player.getActivePotionEffect(BuffList.psychedelic)
                .getAmplifier() == 1) { // only amplified for higher
                    if (player.getActivePotionEffect(BuffList.psychedelic)
                        .getDuration() > 10) higherHallucinationTick(player);
    ...
    private void HandleArmorInfusionExtendedProperties(EntityPlayer player, ExtendedProperties extendedProperties) {
        if (player.worldObj.isRemote) {
            ...
            if (player.ticksExisted % divisor == 0) AMCore.proxy.particleManager.spawnAuraParticles(player);
            AMCore.proxy.setViewSettings();
        }

        if (ArmorHelper.isInfusionPreset(player.getCurrentArmor(1), GenericImbuement.stepAssist)) {
            if (player.stepHeight != 1.0111f) player.stepHeight = 1.0111f;
    ...
    public static EntityCreature summonCreature(World world, Class creatureClass, int x, int y, int z,
        EntityLivingBase target, int minRange, int maxRange) {
        if (!world.isRemote) {
    ...
    public void onEntityHurt(LivingHurtEvent event) {
        ...
        if (entitySource instanceof EntityPlayer && ((EntityPlayer) entitySource).inventory.armorInventory[2] != null
            && ((EntityPlayer) entitySource).inventory.armorInventory[2].getItem()
        ...
        if (!(extendedProperties.getContingencyEffect(4)
            .getItem() instanceof ItemSnowball) && ent.getHealth() <= ent.getMaxHealth() / 3) {
            extendedProperties.procContingency(
                4,
                (event.source.getEntity() != null && event.source.getEntity() instanceof EntityLivingBase)
                    ? (EntityLivingBase) event.source.getEntity()
        ...
        if (entitySource instanceof EntityPlayer && ((EntityPlayer) entitySource).inventory.armorInventory[3] != null
        ...
        if (ent.isPotionActive(BuffList.fury.id)) event.ammount /= 2;

        if (entitySource instanceof EntityLivingBase
            && ((EntityLivingBase) entitySource).isPotionActive(BuffList.shrink)) event.ammount /= 2;
    }
