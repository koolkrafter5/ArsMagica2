...
public class AMLineArc extends EntityFX {
    ...
    public AMLineArc(World world, double x, double y, double z, Entity targetEntity, String IIconName) {
        this(
            ...
            z,
            targetEntity.posX,
            targetEntity.posY + targetEntity.getEyeHeight() - (targetEntity.height * 0.2f),
            targetEntity.posZ,
            IIconName);
    ...
    public AMLineArc(World world, Entity sourceEntity, Entity targetEntity, String IIconName) {
        this(
            world,
            sourceEntity.posX,
            sourceEntity.posY + sourceEntity.getEyeHeight() - (sourceEntity.height * 0.2f),
            sourceEntity.posZ,
            targetEntity.posX,
            targetEntity.posY + targetEntity.getEyeHeight() - (targetEntity.height * 0.2f),
            targetEntity.posZ,
    ...
    public void onUpdate() {
        ...
        if (targetEntity != null) {
            if (ignoreAge) this.ticksExisted = 0;
            if (targetEntity.isDead) {
                this.setDead();
                return;
            }
            targetPoint = new AMVector3(
                targetEntity.posX,
                targetEntity.posY + targetEntity.getEyeHeight() - (targetEntity.height * 0.2f),
                targetEntity.posZ);
            currentTargetPoint = targetPoint.copy();
        ...
        if (extendToTarget && extensionProgress < 1.0f) {
            ...
            AMVector3 delta = targetPoint.copy()
                .sub(sourcePoint);
    ...
    @Override
    public void renderParticle(Tessellator par1Tessellator, float partialTicks, float rotationX, float rotationXZ,
