...
public class ParticleArcToEntity extends ParticleController {
    ...
    private void addParticleAtPoint(AMVector3 point) {
        AMParticle p = (AMParticle) AMCore.instance.proxy.particleManager
            .spawn(particle.worldObj, "hr_smoke", point.x, point.y, point.z);
        if (p != null) {
    ...
    public void doUpdate() {
        ...
        AMVector3 bez = MathUtilities.bezier(
            start,
            firstControl,
            secondControl,
            new AMVector3(target).add(new AMVector3(0.0, target.getEyeHeight(), 0.0)),
            percent);
