...
public class ParticleRenderer {
    ...
    public void addAMParticle(AMParticle particle) {
        if (particle.isRadiant()) deferredRadiants.add(particle);
        else if (particle.isBlockTexture()) deferredBlocks.add(particle);
        else deferredParticles.add(particle);
    }
    ...
    private void renderStandardParticles(float partialTicks) {
        ...
        for (int i = 0; i < particles.size(); i++) {
            if (i >= particles.size()) break; // This seems useless. It isn't. It prevents a ConcurrentMod crash,
                                              // because some threaded nonsense is causing trouble.
            EntityFX particle = particles.get(i);
            tessellator.setBrightness(particle.getBrightnessForRender(partialTicks));

            particle
                .renderParticle(tessellator, partialTicks, rotationX, rotationXZ, rotationZ, rotationYZ, rotationXY);
        }
