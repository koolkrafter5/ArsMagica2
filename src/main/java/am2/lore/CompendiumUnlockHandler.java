...
public class CompendiumUnlockHandler {
    ...
    /**
     * This is a catch all method - it's genericized to attempt to unlock a compendium entry for anything AM2 based that
     * the player picks up
     *
     * @param event
     */
    ...
    public void onEntityDeath(LivingDeathEvent event) {
        if (event.entityLiving.worldObj.isRemote && event.source.getSourceOfDamage() instanceof EntityPlayer) {
            if (event.entity instanceof EntityEnderman) {
                ...
                EntityRegistration reg = EntityRegistry.instance()
                    .lookupModSpawn(event.entityLiving.getClass(), true);
                if (reg != null && reg.getContainer()
                    .matches(AMCore.instance)) {
    ...
    public void onSpellCast(SpellCastingEvent.Pre event) {
        if (event.caster == AMCore.proxy.getLocalPlayer()) {
            ...
            if (ExtendedProperties.For(event.caster)
                .getCurrentMana()
                < ExtendedProperties.For(event.caster)
                    .getMaxMana() / 2)
    ...
    /**
     * Helper method (auto-proxied) that will unlock a compendium entry. If the entry is found to be a category, it wil
     * be unlocked instead.
     *
