...
public class CompendiumEntryBlock extends CompendiumEntry {
    ...
    protected GuiArcaneCompendium getCompendiumGui(String searchID, int meta) {
        ...
        if (idSplit.length == 2) {
            ...
            if (block != null) {
                if (meta == -1) return new GuiArcaneCompendium(block);
                else return new GuiArcaneCompendium(searchID + "@" + meta, block, meta);
            }
        } else {
            for (Block block : AMCore.instance.proxy.blocks.getArsMagicaBlocks()) {
                ...
                if (block.getUnlocalizedName()
                    ...
                    .equals(split[0])) {
                    if (meta == -1) return new GuiArcaneCompendium(block);
                    else return new GuiArcaneCompendium(this.id, block, meta);
                }
            }
        }
        ...
        if (idSplit.length == 2) {
            ...
            for (Block block : AMCore.instance.proxy.blocks.getArsMagicaBlocks()) {
                ...
                if (block.getUnlocalizedName()
                    ...
                    .replace("tile.", "")
                    .equals(split[0])) {
                    if (meta == -1) return new ItemStack(block);
                    else return new ItemStack(block, 1, meta);
                }
            }
        }
