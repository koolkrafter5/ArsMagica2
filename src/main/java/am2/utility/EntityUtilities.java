...
public class EntityUtilities {
    ...
    public static void makeSummon_PlayerFaction(EntityCreature entityliving, EntityPlayer player,
        ...
        if (isAIEnabled(entityliving) && !(entityliving instanceof IBossDisplayData)
            ...
            if (addMeleeAttack) {
                ...
                entityliving.tasks
                    .addTask(3, new EntityAIAttackOnCollide(entityliving, (Class) IMob.class, speed, true));
                entityliving.tasks
    ...
    public static boolean isTileSpawnedAndValid(EntityLivingBase entityliving) {
        Integer tileX = entityliving.getEntityData()
            .getInteger(summonTileXKey);
        Integer tileY = entityliving.getEntityData()
            .getInteger(summonTileYKey);
        Integer tileZ = entityliving.getEntityData()
    ...
    public static int getOwner(EntityLivingBase entityliving) {
        ...
        Integer ownerID = entityliving.getEntityData()
    ...
    public static void setSize(EntityLivingBase entityliving, float width, float height) {
        if (entityliving.width == width && entityliving.height == height) return;
        if (ptrSetSize == null) {
            try {
                ptrSetSize = ReflectionHelper.findMethod(
                    Entity.class,
                    entityliving,
                    new String[] { "func_70105_a", "setSize" },
                    Float.TYPE,
                    Float.TYPE);
            } catch (Throwable t) {
                t.printStackTrace();
