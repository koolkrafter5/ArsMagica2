...
public class MathUtilities {
    ...
    public static AMVector3[] GetHorizontalBlocksInFrontOfCharacter(EntityLivingBase entity, int numBlocks, int x,
        int y, int z) {
        float speed = 0.1F;
    ...
    public static Entity[] GetEntitiesInAngleNearEntity(World world, EntityLivingBase source, int degrees, int radius,
        Class filterClass, boolean includeSource) {
        ...
        List distanceFilter = world.getEntitiesWithinAABB(
            filterClass,
            AxisAlignedBB.getBoundingBox(
                (float) source.posX - radius,
                source.posY - radius,
                (float) source.posZ - radius,
                source.posX + radius,
                source.posY + radius,
                source.posZ + radius));
        if (!includeSource) {
    ...
    public static boolean isInFieldOfVision(Entity e1, EntityLivingBase e2, float angleX, float angleY) {
        ...
        if (e2 instanceof EntityLiving) ((EntityLiving) e2).faceEntity(e1, 360F, 360F);
        ...
        // assuming field of vision consists of everything within X degrees from rotationYaw and Y degrees from
        // rotationPitch, check if entity 2's current rotationYaw and rotationPitch within this X and Y range
        float X = angleX;
        float Y = angleY;
    ...
    public static Entity getPointedEntity(World world, EntityLivingBase entityplayer, double range,
        ...
        double d = range;
        Vec3 vec3d = Vec3
            .createVectorHelper(entityplayer.posX, entityplayer.posY + entityplayer.getEyeHeight(), entityplayer.posZ);
