...
public class AMTeleporter extends Teleporter {
    ...
    public void teleport(EntityLivingBase entity, World world) {
        ...
        // AMCore.log.info("Teleported to dim " + world.provider.dimensionId + ": " + teleportPos.x + "/" +
        // teleportPos.y + "/" + teleportPos.z);
    }
    ...
    private AMVector3 clearTeleportPath(World world, EntityLivingBase entity) {
        ...
        if (entity.dimension != -1) {
            ...
            if (vec.y < 5 || vec.y >= world.getActualHeight() - 10) vec.y = 5;

            while (true) {
                if (world.getBlock((int) vec.x, (int) vec.y, (int) vec.z) == Blocks.air
                    || vec.y >= world.getActualHeight()) {
            ...
            if (canFindHigherGround) {
                ...
                if (vec.y < 5) vec.y = 5;
                if (vec.y > world.getActualHeight() - 10) vec.y = world.getActualHeight() - 10;
