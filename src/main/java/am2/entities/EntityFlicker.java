...
public class EntityFlicker extends EntityAmbientCreature {
    ...
    public void onUpdate() {
        ...
        if (this.ticksExisted > 100 && playerClose
            && this.dataWatcher.getWatchableObjectByte(WATCHER_AMBIENTFLICK) == (byte) 0) {
            ...
            flickCount++;
            if (worldObj.isRemote && flickCount > 7) flick(); // client flick
            else if (!worldObj.isRemote && flickCount > 10) flick(); // server flick
        }
        ...
        if (worldObj.isRemote) {
            ...
            if (getRNG().nextInt(10) < AMCore.config.getGFXLevel()) {
                AMParticle effect = (AMParticle) AMCore.proxy.particleManager.spawn(
                    worldObj,
                    AMParticleIcons.instance.getParticleForAffinity(getFlickerAffinity()),
                    posX,
                    posY,
                    posZ);
                if (effect != null) {
                    ...
                    else effect.setParticleScale(0.05f + rand.nextFloat() * 0.05f);
                }
            }
        }
    }
    ...
    protected void updateEntityActionState() {
        ...
        if (needsNewPath && worldObj.rand.nextDouble() < 0.1f) pickNewTargetPosition();
        ...
        normalizedMovementVector = me.copy()
            .sub(targetPosition)
            .normalize();

        if (normalizedMovementVector.y > 0) rotatePitchTowards(-70 * normalizedMovementVector.y, 30);
        else rotatePitchTowards(0, 30);

        float speed = 0.2f;
        this.addVelocity(
            -normalizedMovementVector.x * speed,
            -normalizedMovementVector.y * speed,
        ...
        if (this.worldObj.isRemote) {
            for (int i = 0; i < 10 * AMCore.config.getGFXLevel(); ++i) {
                AMParticle particle = (AMParticle) AMCore.proxy.particleManager
                    .spawn(worldObj, "radiant", posX, posY, posZ);
                if (particle != null) {
    ...
    private void pickNewTargetPosition() {
        ...
        switch (this.getFlickerAffinity()) {
            case WATER: // water flicker
                for (int i = 0; i < 5; ++i) {
                    targetPosition = new AMVector3(
                        this.posX - 5 + worldObj.rand.nextInt(10),
                        this.posY - 5 + worldObj.rand.nextInt(10),
                        this.posZ - 5 + worldObj.rand.nextInt(10));
            ...
            case AIR: // air flicker
                ...
                targetPosition = new AMVector3(
                    this.posX - 5 + worldObj.rand.nextInt(10),
                    groundLevel + 10 + worldObj.rand.nextInt(15),
                    this.posZ - 5 + worldObj.rand.nextInt(10));
                break;
            case EARTH: // earth flicker
                ...
                targetPosition = new AMVector3(
                    this.posX - 5 + worldObj.rand.nextInt(10),
                    groundLevel + worldObj.rand.nextInt(1) + 1,
                    this.posZ - 5 + worldObj.rand.nextInt(10));
                break;
            default: // all others
                groundLevel = getTopBlockNearMe();
                targetPosition = new AMVector3(
                    this.posX - 5 + worldObj.rand.nextInt(10),
                    groundLevel + 3 + worldObj.rand.nextInt(5),
                    this.posZ - 5 + worldObj.rand.nextInt(10));
        }
    }
    ...
    private int getTopBlockNearMe() {
        ...
        while (y > 0 && worldObj.isAirBlock(x, y, z)) y--;
        while (y < worldObj.getActualHeight() && !worldObj.isAirBlock(x, y, z)) y++;
            ...
            if (stack.getItemDamage() == 0) {
                if (!worldObj.isRemote) {
                    ...
                    if (!InventoryUtilities.mergeIntoInventory(player.inventory, newStack)) {
                        if (!worldObj.isRemote) player.dropItem(newStack.getItem(), newStack.getItemDamage());
                    }
                }
                return true;
            } else {
    ...
    public boolean getCanSpawnHere() {
        ...
        if (AMCore.proxy.getTotalFlickerCount() > 8 * worldObj.playerEntities.size()
        ...
        if (biome != null) {
            ...
            if (validAffinities.size() <= 0) return false;
