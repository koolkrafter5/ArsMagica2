...
public class EntityWhirlwind extends EntityFlying {
            ...
            if (cd == null || cd <= 0) {
                if (!worldObj.isRemote && rand.nextInt(100) < 10) {
                    ...
                    if (player.inventory.getStackInSlot(slot) != null) {
                        ItemStack armorStack = player.inventory.getStackInSlot(slot)
                            .copy();
    ...
    public void onUpdate() {
        if (currentTarget == null || new AMVector3(this).distanceSqTo(currentTarget) < 2) generateNewTarget();

        nav.tryMoveFlying(worldObj, this);

        if (this.ticksExisted > 140 && !this.worldObj.isRemote) this.setDead();
    ...
    private void generateNewTarget() {
        ...
        if (closest != null && this.getDistanceSqToEntity(closest) < 64D) currentTarget = new AMVector3(closest);
        else currentTarget = new AMVector3(this)
