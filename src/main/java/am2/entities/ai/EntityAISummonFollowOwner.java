...
public class EntityAISummonFollowOwner extends EntityAIBase {
    ...
    public void startExecuting() {
        ...
        this.field_75344_i = this.theSummon.getNavigator()
            .getAvoidsWater();
        this.theSummon.getNavigator()
                        ...
                        for (int l = 0; l <= 4; ++l) {
                            for (int i1 = 0; i1 <= 4; ++i1) {
                                if ((l < 1 || i1 < 1 || l > 3 || i1 > 3)
                                    ...
                                    && !this.theWorld.getBlock(i + l, k, j + i1)
                                        .isNormalCube()
                                    && !this.theWorld.getBlock(i + l, k + 1, j + i1)
                                        .isNormalCube()) {
                                    this.theSummon.setLocationAndAngles(
                                        (double) ((float) (i + l) + 0.5F),
                                        (double) k,
                                        (double) ((float) (j + i1) + 0.5F),
                                        this.theSummon.rotationYaw,
                                        this.theSummon.rotationPitch);
                                    this.petPathfinder.clearPathEntity();
                                    return;
                                }
                            }
