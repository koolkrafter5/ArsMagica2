...
public class EntityAITargetNearbyInanimate extends EntityAITarget {
    ...
    public boolean shouldExecute() {
        return (taskOwner.getAttackTarget() == null && ExtendedProperties.For(taskOwner)
            .getInanimateTarget() == null);
    }

    @Override
    public boolean continueExecuting() {
        Entity inanimateTarget = ExtendedProperties.For(taskOwner)
            .getInanimateTarget();
    ...
    public void resetTask() {
        ExtendedProperties.For(taskOwner)
            .setInanimateTarget(null);
        this.target = null;
    }

    protected boolean isSuitableTarget(Entity target) {
        if (target.isDead) return false;
        for (Class c : targetTypes) if (target.getClass() == c) return true;
        return false;
    }
    ...
    public void startExecuting() {
        ...
        for (Class c : targetTypes) {
            List<Entity> potentialTargets = taskOwner.worldObj
                .getEntitiesWithinAABB(c, taskOwner.boundingBox.expand(targetDistance, 1, targetDistance));
        ...
        if (this.target != null) ExtendedProperties.For(taskOwner)
            .setInanimateTarget(this.target);
