...
public class EntityAITargetByTeam extends EntityAITarget {
    ...
    private boolean tryGetTargetByTeam() {
        List<EntityPlayer> players = host.worldObj
            .getEntitiesWithinAABB(EntityPlayer.class, host.boundingBox.expand(radius, radius, radius));

        for (EntityPlayer player : players) {
            if (player.isOnTeam(teamToTarget)) {
                if (host.getNavigator()
                    .tryMoveToEntityLiving(player, host.getAIMoveSpeed())) {
