...
public class EntityAIManaDrainBolt extends EntityAIBase {
    ...
    public boolean continueExecuting() {
        return shouldExecute() || !entityHost.getNavigator()
            .noPath();
    }
    ...
    public void updateTask() {
        ...
        boolean flag = entityHost.getEntitySenses()
            .canSee(attackTarget);
        ...
        if (d1 > d || stuckTime < 20) {
            entityHost.getNavigator()
                .tryMoveToEntityLiving(attackTarget, moveSpeed);
        } else {
            entityHost.getNavigator()
                .clearPathEntity();
        }

        entityHost.getLookHelper()
    ...
    private void doRangedAttack() {
        ...
        if (worldObj.rand.nextInt(100) < chanceToMiss) {
            ...
            AMCore.instance.proxy.particleManager
                .BoltFromEntityToEntity(worldObj, entityHost, entityHost, attackTarget, this.damage, 2, -1);
            float manaDrained = this.manaDrainedPerCasterLevel * ExtendedProperties.For(attackTarget)
                .getMagicLevel();
            ExtendedProperties.For(attackTarget)
                .setCurrentMana(
                    ExtendedProperties.For(attackTarget)
                        .getCurrentMana() - (manaDrained));
            ExtendedProperties.For(attackTarget)
                .forceSync();
