...
public class EntityAIAllyManaLink extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        if (owner == null || !SkillData.For(owner)
            .isEntryKnown(SkillTreeManager.instance.getSkillTreeEntry(SkillManager.instance.getSkill("MageBandII")))
            || host.getDistanceSqToEntity(host) > 64D
            || ExtendedProperties.For(owner)
                .isManaLinkedTo(host))
            return false;
        return true;
    }
    ...
    private EntityPlayer getHostOwner() {
        ...
        if (owner == null || !(owner instanceof EntityPlayer)) return null;
    ...
    public void updateTask() {
        ...
        if (host.getDistanceToEntity(owner) < 1) host.getNavigator()
            .tryMoveToXYZ(host.posX, host.posY, host.posZ, 0.5f);
