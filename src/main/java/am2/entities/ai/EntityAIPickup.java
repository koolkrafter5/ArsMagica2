...
public class EntityAIPickup extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        Entity target = ExtendedProperties.For(host)
            .getInanimateTarget();
        if (!(target instanceof EntityItem)) return false;
    ...
    public void updateTask() {
        ...
        if (lastLocation.distanceSqTo(new AMVector3(target)) > 4) {
            if (!host.getNavigator()
                .tryMoveToEntityLiving(target, 0.4f)
                || !host.getEntitySenses()
                    .canSee(target)) {
                ExtendedProperties.For(host)
                    .setInanimateTarget(null);
                resetTask();
