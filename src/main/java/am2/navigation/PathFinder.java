...
public final class PathFinder {
    ...
    private static BreadCrumb FindPathReversed(World world, Point3D start, Point3D end, Entity entity)
        ...
        // shift the coordinates so that they are based on a zero-index (this is because we use the point coordinates as
        // an index into the path array)
        if (start.x < end.x) {
        ...
        while (openList.Count() > 0) {
            // Find best item and switch it to the 'closedList' - best being the item currently on the open list with
            // the lowest cost
    ...
    private static boolean isDiagonalMovement(Point3D start, Point3D end) {
        ...
        return (hasDeltaX && hasDeltaY) || (hasDeltaX && hasDeltaZ)
            || (hasDeltaZ && hasDeltaY)
    ...
    private static Point3D[] surrounding = new Point3D[] {
        // Top slice (Y=1)
        new Point3D(-1, 1, 1), new Point3D(0, 1, 1), new Point3D(1, 1, 1), new Point3D(-1, 1, 0), new Point3D(0, 1, 0),
        new Point3D(1, 1, 0), new Point3D(-1, 1, -1), new Point3D(0, 1, -1), new Point3D(1, 1, -1),
        // Middle slice (Y=0)
        new Point3D(-1, 0, 1), new Point3D(0, 0, 1), new Point3D(1, 0, 1), new Point3D(-1, 0, 0), new Point3D(1, 0, 0), // (0,0,0)
                                                                                                                        // is
                                                                                                                        // self
