...
public class PathNavigator {
    ...
    private void MoveEntityTowardBreadCrumbFlying() {
        ...
        AMVector3 target = new AMVector3(
            pathData.position.x + 0.5f,
            pathData.position.y + 0.5f,
            pathData.position.z + 0.5f);

        AMVector3 movement = target.sub(current)
            .normalize();
        ...
        if (distance < 0.8f) {
            ...
            if (pathData != null) {
                ...
                if (pathIsLongRange) {
                    SetWaypoint(
                        world,
                        this.longRangeWaypoint.x,
                        this.longRangeWaypoint.y,
                        this.longRangeWaypoint.z,
                        entity);
                    GetPathToWaypoint(world, entity);
                }
            }
    ...
    private void getEntityLocation(World world) {
        ...
        currentLocation = new Point3D(
            (int) Math.floor(pathEntity.posX),
            (int) Math.floor(pathEntity.posY),
            (int) Math.floor(pathEntity.posZ));
        if (!BlockIsAir(world, currentLocation)) {
            currentLocation = new Point3D(
                (int) Math.ceil(pathEntity.posX),
                (int) Math.floor(pathEntity.posY),
                (int) Math.floor(pathEntity.posZ));
            if (!BlockIsAir(world, currentLocation)) {
                currentLocation = new Point3D(
                    (int) Math.floor(pathEntity.posX),
                    (int) Math.ceil(pathEntity.posY),
                    (int) Math.floor(pathEntity.posZ));
                if (!BlockIsAir(world, currentLocation)) {
                    currentLocation = new Point3D(
                        (int) Math.floor(pathEntity.posX),
