...
public class GuiArcaneCompendium extends GuiScreen {
                                ...
                                try {
                                    ...
                                    recipe.add(
                                        new ItemStack(
                                            ...
                                            amount,
                                            ItemsCommonProxy.essence.META_MAX + type));
                                } catch (Throwable t) {
                                    continue;
                                }
                            } else {
    ...
    protected void actionPerformed(GuiButton par1GuiButton) {
        switch (par1GuiButton.id) {
            ...
            case 2:
                ...
                if (prevEntry != null) {
                    if (prevEntry.entryType == prevEntry.TYPE_ENTRY) Minecraft.getMinecraft()
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        if (this.page == 0) prevPage.visible = false;
        else prevPage.visible = true;

        if (this.page == numPages) nextPage.visible = false;
    ...
    private void drawRightPage(int l, int i1, int mousex, int mousey) {
        ...
        if (modifiers.size() > 0) drawModifiers(cx, entryMultiblock == null ? cy : cy + 20, mousex, mousey);
    }
            ...
            for (int k = 0; k < list.size(); ++k) {
                if (k == 0) {
                    if (entry != null) {
                        ...
                        list.set(
                            k,
                            "\u00a7" + stack.getRarity().rarityColor.getFormattingCode() + (String) list.get(k));
                    }
                } else {
    ...
    private void renderCraftingComponent(int index, int sx, int sy, int mousex, int mousey) {
        ...
        if (craftingComponent instanceof ItemStack) {
            ...
            if (((List) craftingComponent).size() == 0) return;
