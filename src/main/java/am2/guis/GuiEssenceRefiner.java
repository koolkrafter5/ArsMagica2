...
public class GuiEssenceRefiner extends GuiContainer {
    ...
    private static final ResourceLocation background = new ResourceLocation(
        "arsmagica2",
        ResourceManager.GetGuiTexturePath("essenceExtractorGui.png"));
    private static final ResourceLocation extras = new ResourceLocation(
        "arsmagica2",
        ResourceManager.GetGuiTexturePath("essenceExtractorGui_2.png"));
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
        ...
        if (active && PowerNodeRegistry.For(Minecraft.getMinecraft().theWorld)
            .getHighestPower(essenceExtractorInventory) >= essenceExtractorInventory.TICK_REFINE_COST)
            this.rotation += 0.05f;
        ...
        if (essenceExtractorInventory.isRefining() && PowerNodeRegistry.For(Minecraft.getMinecraft().theWorld)
            ...
            GL11.glColor4f(
                baseColorRed + (colorShiftRed * percentComplete),
                baseColorGreen + (colorShiftGreen * percentComplete),
                baseColorBlue + (colorShiftBlue * percentComplete),
                0.5f);
        }
