...
public class GuiSkillTrees extends GuiScreen {
    ...
    private static final ResourceLocation rl_background = new ResourceLocation(
        "arsmagica2",
        "textures/guis/SkillTreeUI.png");
    private static final ResourceLocation rl_offense = new ResourceLocation(
        "arsmagica2",
        "textures/guis/SkillTree_Offense.png");
    private static final ResourceLocation rl_defense = new ResourceLocation(
        "arsmagica2",
        "textures/guis/SkillTree_Defense.png");
    private static final ResourceLocation rl_utility = new ResourceLocation(
        "arsmagica2",
        "textures/guis/SkillTree_Utility.png");
    private static final ResourceLocation rl_talents = new ResourceLocation(
        "arsmagica2",
        "textures/guis/SkillTree_Talents.png");
    ...
    public void initGui() {
        ...
        defense = new GuiButtonSkillTreeTab(
            0,
            l + GuiButtonSkillTreeTab.buttonWidth + buttonPadding,
            i1,
            SkillTrees.Defense);
        utility = new GuiButtonSkillTreeTab(
            0,
            l + GuiButtonSkillTreeTab.buttonWidth * 2 + buttonPadding * 2,
            i1,
            SkillTrees.Utility);
        talents = new GuiButtonSkillTreeTab(
            0,
            l + GuiButtonSkillTreeTab.buttonWidth * 3 + buttonPadding * 3,
            i1,
            SkillTrees.Talents);
        affinity = new GuiButtonSkillTreeTab(
            0,
            l + GuiButtonSkillTreeTab.buttonWidth * 4 + buttonPadding * 4,
            i1,
            SkillTrees.Affinity);
        ...
        if (ExtendedProperties.For(Minecraft.getMinecraft().thePlayer)
            .getMagicLevel() >= 5) this.buttonList.add(talents);
        if (!ArcaneCompendium.instance.getEntry("affinity")
            .isLocked()) this.buttonList.add(affinity);
    }
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        if (AMCore.config.getSkillTreeSecondaryTierCap() < SkillTreeManager.instance.getHighestTier()
            && (sk.getPrimaryTree() == null || sk.getPrimaryTree() == SkillTrees.None)
            && sk.getSpellPoints(SkillPointTypes.BLUE) > 0) {
        ...
        if (activeTree == SkillTrees.Offense) Minecraft.getMinecraft().renderEngine.bindTexture(rl_offense);
        else if (activeTree == SkillTrees.Defense) Minecraft.getMinecraft().renderEngine.bindTexture(rl_defense);
        else if (activeTree == SkillTrees.Utility) Minecraft.getMinecraft().renderEngine.bindTexture(rl_utility);
        ...
        drawTexturedModalRect_Classic(
            l + 5,
            i1 + GuiButtonSkillTreeTab.buttonHeight + 5,
            offsetX,
            offsetY,
            200,
            200,
        ...
        Minecraft.getMinecraft().fontRenderer.drawString(
            quantity,
            l + xSize - 24 - (Minecraft.getMinecraft().fontRenderer.getStringWidth(quantity) / 2),
            i1 + 5,
            0x0000FF);
        ...
        Minecraft.getMinecraft().fontRenderer.drawString(
            quantity,
            l + xSize - 12 - (Minecraft.getMinecraft().fontRenderer.getStringWidth(quantity) / 2),
            i1 + 5,
            0x00FF00);
        ...
        Minecraft.getMinecraft().fontRenderer.drawString(
            quantity,
            l + xSize - 18 - (Minecraft.getMinecraft().fontRenderer.getStringWidth(quantity) / 2),
            i1 + 15,
            0xFF0000);
        ...
        if (hoveredItem != null) {
            ...
            if (!AMCore.config.colourblindMode()) {
                drawHoveringText(
                    ...
                    lastMouseY,
                    fr,
                    state == LearnStates.LEARNED ? 0xFFFFFF
                        : type == SkillPointTypes.SILVER ? 0x888888
                            : type == SkillPointTypes.BLUE ? 0x4444FF
                                : type == SkillPointTypes.GREEN ? 0x44FF44 : 0xFF4444);
            } else {
                text.add(
                    StatCollector.translateToLocal(
                        "am2.gui." + type.toString()
                            .toLowerCase() + "Point"));
        ...
        if (hoveredAffinity != null) {
            List list = AffinityData.For(Minecraft.getMinecraft().thePlayer)
                .getColoredAffinityEffects(hoveredAffinity);
    ...
    private void drawSkillTree() {
        ...
        for (SkillTreeEntry entry : skillTree) {
            ...
            for (SkillTreeEntry prerequisite : entry.prerequisites) {
                int color = 0;
                if (sk.isEntryKnown(prerequisite)
                    || sk.getLearnState(prerequisite, Minecraft.getMinecraft().thePlayer) == LearnStates.DISABLED) {
                    color = 0x006600;
                } else {
                    continue;
                }
                line2d(
                    l + entry.x - offsetX * 2 + 16,
                    i1 + entry.y - offsetY * 2 + 16,
                    l + entry.x - offsetX * 2 + 16,
                    i1 + prerequisite.y - offsetY * 2 + 16,
                    color);
                line2d(
                    l + entry.x - offsetX * 2 + 16,
                    i1 + prerequisite.y - offsetY * 2 + 16,
                    l + prerequisite.x - offsetX * 2 + 16,
                    i1 + prerequisite.y - offsetY * 2 + 16,
                    color);
            }
        }
        ...
        for (SkillTreeEntry entry : skillTree) {
            ...
            for (SkillTreeEntry prerequisite : entry.prerequisites) {
                ...
                if ((sk.isEntryKnown(prerequisite) && (state == LearnStates.CAN_LEARN || state == LearnStates.LEARNED))
                    || sk.getLearnState(prerequisite, Minecraft.getMinecraft().thePlayer) == LearnStates.DISABLED) {
                    continue;
                } else {
                    color = 0x220000;
        ...
        for (SkillTreeEntry entry : skillTree) {
            ...
            if (state == LearnStates.LEARNED) {
                ...
                if (type == SkillPointTypes.SILVER) {
                    ...
                    GL11.glColor3f(
                        min + delta + ((type == SkillPointTypes.RED) ? colorShift : 0f),
            ...
            DrawConstrainedIconAtXY(
                IIcon,
                l + entry.x - offsetX * 2,
                i1 + entry.y - offsetY * 2,
                IIcondims,
                IIcondims,
                false);
            ...
            if (state == LearnStates.LOCKED) {
                ...
                DrawConstrainedIconAtXY(
                    AMGuiIcons.padlock,
                    l + entry.x - offsetX * 2 + 8.5f,
                    i1 + entry.y - offsetY * 2 + 8,
                    15,
                    15,
                    true);
            ...
            if (lastMouseX > l && lastMouseX < l + xSize && lastMouseY > i1 && lastMouseY < i1 + ySize) {
                if (lastMouseX > l + entry.x - offsetX * 2 && lastMouseX < l + entry.x - offsetX * 2 + IIcondims) {
                    if (lastMouseY > i1 + entry.y - offsetY * 2
                        && lastMouseY < i1 + entry.y - offsetY * 2 + IIcondims) {
                        hovered = entry;
                    }
                }
            }
    ...
    private void drawAffinity() {
        ...
        for (Affinity aff : Affinity.getOrderedAffinities()) {
            ...
            if (ad.getAffinityDepth(aff) > 0.01f) {
                AMGuiHelper.instance.fractalLine2d(
                    lx + halfIconSize,
                    ly + halfIconSize,
                    cx1 + halfIconSize,
                    cy1 + halfIconSize,
            ...
            fontRendererObj.drawString(
                depthString,
                newX + halfIconSize - fontRendererObj.getStringWidth(depthString) / 2,
                newY + IIconsize,
                aff.color,
                false);
            angle += 36;
    ...
    protected void mouseClicked(int par1, int par2, int par3) {
        ...
        if (par3 == 0) {
            if (hoveredItem != null) {
                ...
                if (this.activeTree != SkillTrees.Affinity) isDragging = true;
                lastMouseX = par1;
                lastMouseY = par2;
            }
        }
