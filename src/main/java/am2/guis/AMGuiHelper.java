...
public class AMGuiHelper {
    ...
    public CompendiumBreadcrumb popCompendiumBreadcrumb() {
        if (compendiumBreadcrumbs.size() > 0) return compendiumBreadcrumbs.pollLast();
        return null;
    }
    ...
    public static void DrawIconAtXY(IIcon IIcon, float x, float y, float zLevel, int w, int h,
        ...
        boolean drawing = ReflectionHelper
            .getPrivateValue(Tessellator.class, tessellator, "field_78415_z", "isDrawing");
        ...
        if (drawing) tessellator.startDrawingQuads();
    }
    ...
    protected static void drawHoveringText(List par1List, int par2, int par3, FontRenderer font, int width,
        int height) {
        if (!par1List.isEmpty()) {
