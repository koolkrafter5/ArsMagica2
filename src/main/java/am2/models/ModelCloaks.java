...
public class ModelCloaks extends ModelBase {
    ...
    public void render(EntityPlayer player, ModelBiped mainModel, float f5, float partialTicks,
        ...
        if (cloakLocation == null) return;

        float fr = ExtendedProperties.For(player)
            .getFlipRotation();
        if (fr > 0) {
            return;
        }
        ...
        float pct = ExtendedProperties.For(player)
            .getShrinkPct();
        if (pct > 0) {
            float amt = 0.5f * pct;
        ...
        if ((drawMask & MASK_SLEEVES) == MASK_SLEEVES) LeftSleeveLong.render(f5);
        else LeftSleeveShort.render(f5);
        GL11.glTranslatef(offset * 2, 0, 0);
        if ((drawMask & MASK_SLEEVES) == MASK_SLEEVES) RightSleeveLong.render(f5);
        else RightSleeveShort.render(f5);
