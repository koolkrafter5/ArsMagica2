...
public class PowerNodeCache {
    ...
    public NBTTagCompound getNBTForChunk(World world, ChunkCoordIntPair chunk) {
        ...
        if (dataCache.containsKey(rc)) {
            ...
            if (compound.hasKey("AM2PowerData")) return compound;
            dataCache.remove(rc);
        }
    ...
    public void onChunkUnload(ChunkEvent.Unload event) {
        if (!event.world.isRemote && PowerNodeRegistry.For(event.world)
            .hasDataForChunk(event.getChunk())) {
            NBTTagCompound dataCompound = new NBTTagCompound();
            PowerNodeRegistry.For(event.world)
                .SaveChunkToNBT(
                    event.getChunk()
    ...
    public void onWorldSave(WorldEvent.Save event) {
        ...
        HashMap<ChunkCoordIntPair, NBTTagCompound> saveData = PowerNodeRegistry.For(world)
            .saveAll();
    ...
    private void cacheToFile(World world, RegionCoordinates coords) {
        ...
        if (!cachedRegion.hasKey("AM2PowerData")) return;
    ...
    public void saveWorldToFile(World world) {
        ...
        // cached data to file
        Iterator<RegionCoordinates> it = dataCache.keySet()
            .iterator();
        while (it.hasNext()) {
