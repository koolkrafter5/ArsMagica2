...
public class PowerNodePathfinder extends AStar<AMVector3> {
    ...
    private IPowerNode getPowerNode(World world, AMVector3 location) {
        if (world.checkChunksExist(
            ...
            if (chunk.isChunkLoaded) {
                ...
                if (te instanceof IPowerNode) return (IPowerNode) te;
            }
        }
        return null;
    }
    ...
    protected List<AMVector3> generateSuccessors(AMVector3 node) {
        ...
        if (powerNode == null) return new ArrayList<AMVector3>();

        IPowerNode[] candidates = PowerNodeRegistry.For(world)
            .getAllNearbyNodes(world, node, powerType);
    ...
    private boolean verifyCandidate(IPowerNode powerNode) {
        if (new AMVector3((TileEntity) powerNode).equals(end)) {
            for (PowerTypes type : powerNode.getValidPowerTypes()) if (type == powerType) return true;
        }
