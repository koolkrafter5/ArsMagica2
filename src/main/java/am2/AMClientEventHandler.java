...
public class AMClientEventHandler {
    ...
    public void onPlayerRender(RenderPlayerEvent.Post event) {
        ModelBiped mainModel = ReflectionHelper
            .getPrivateValue(RenderPlayer.class, event.renderer, "field_77109_a", "modelBipedMain");
        if (mainModel != null) {
    ...
    public void onEntityJoinWorldClient(EntityJoinWorldEvent event) {
        ...
        if (event.entity instanceof EntityPlayer) {
            ...
            if (tablist.contains(Minecraft.getMinecraft().thePlayer.getDisplayName()))
                tablist.remove(Minecraft.getMinecraft().thePlayer.getDisplayName());
            if (tablist.contains(Minecraft.getMinecraft().thePlayer.getCommandSenderName()))
                tablist.remove(Minecraft.getMinecraft().thePlayer.getCommandSenderName());
            if (event.entity == Minecraft.getMinecraft().thePlayer && tablist.size() == 0) { // no players to sync the
                                                                                             // map from, clear it
                                                                                             // instead
                MysteriumPatchesFixesMagicka.playerModelMap.clear();
                return;
