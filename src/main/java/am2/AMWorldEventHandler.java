...
public class AMWorldEventHandler {
    ...
    public void onChunkSaved(ChunkDataEvent.Save event) {
        ...
        if (event.getData()
            .hasKey("ArsMagica2")) {
            compound = event.getData()
                .getCompoundTag("ArsMagica2");
        }
        compound.setBoolean(genKey, true);
        event.getData()
            .setTag("ArsMagica2", compound);
    }

    @SubscribeEvent
    public void onChunkLoaded(ChunkDataEvent.Load event) {
        int dimensionID = event.world.provider.dimensionId;
        ChunkCoordIntPair chunkLocation = event.getChunk()
            .getChunkCoordIntPair();

        NBTTagCompound compound = (NBTTagCompound) event.getData()
            .getTag("ArsMagica2");
