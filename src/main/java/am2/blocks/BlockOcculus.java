...
public class BlockOcculus extends AMSpecialRenderBlockContainer {
    ...
    public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer,
        ...
        if (par1World.isRemote) {
            if (ExtendedProperties.For(par5EntityPlayer)
                .getMagicLevel() > 0) AMCore.proxy.openSkillTreeUI(par1World, par5EntityPlayer);
            else par5EntityPlayer
                .addChatMessage(new ChatComponentText("You cannot comprehend what you see inside the occulus."));
        }
        return true;
    }
