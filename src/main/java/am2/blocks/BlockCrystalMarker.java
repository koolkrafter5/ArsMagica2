...
public class BlockCrystalMarker extends BlockContainer {
    ...
    public TileEntity createNewTileEntity(World world, int i) {
        ...
        if (meta == META_SPELL_EXPORT) return new TileEntityCrystalMarkerSpellExport(meta);
        return new TileEntityCrystalMarker(meta);
    }

    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int facing, float impx,
            ...
            .getItem() == ItemsCommonProxy.spellStaffMagitech) {
                ...
                if (crystalMarkerTE != null) {
                    ...
                    if (elementalAttunerVector != null) {
                        // if we are here, there is an attached attuner to this marker. Resolve the location into the
                        // tile entity.
                        elementalAttunerTE = GetElementalAttunerTileEntity(
                            world,
                        ...
                        if (elementalAttunerTE != null) {
                            ...
                            elementalAttunerTE.switchMarkerPriority(
                                new AMVector3(x, y, z),
                                oldPriority,
                    ...
                    player.addChatMessage(
                        new ChatComponentText(
                            String.format(
                                ...
                                String.format("%d", crystalMarkerTE.getPriority()) // need to put this as a string,
                                                                                   // because for some reason %d doesn't
                                                                                   // work when used in a localized
                                                                                   // string, but %s does
                    )));
                }

                return false;
            } else if (operandType == this.META_SET_EXPORT || operandType == this.META_REGULATE_EXPORT
                || operandType == this.META_REGULATE_MULTI
                || operandType == this.META_SET_IMPORT) {
                    FMLNetworkHandler
                        .openGui(player, AMCore.instance, ArsMagicaGuiIdList.GUI_CRYSTAL_MARKER, world, x, y, z);
                    return true;
                }

        return super.onBlockActivated(world, x, y, z, player, facing, impx, impy, impz);
    }
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess par1iBlockAccess, int x, int y, int z) {
        ...
        switch (facing) {
            case 0: // Bottom, Inventory is above
                this.setBlockBounds(
                    0.35f,
                    (float) ((1 + cm.GetConnectedBoundingBox().minY)) - 0.1f,
                    0.32f,
                    0.65f,
                    (float) ((1 + cm.GetConnectedBoundingBox().minY)),
                    0.68f);
                break;
            case 1: // Top, Inventory is below
                this.setBlockBounds(
                    0.35f,
                    (float) (-1 * (1 - cm.GetConnectedBoundingBox().maxY)),
                    0.3f,
                    0.65f,
                    (float) (-1 * (1 - cm.GetConnectedBoundingBox().maxY)) + 0.1f,
                    0.7f);
                break;
            case 2: // North, Inventory is to the south
                this.setBlockBounds(
                    0.35f,
                    0.32f,
                    (float) (1 + (1 - cm.GetConnectedBoundingBox().maxZ)) - 0.1f,
                    0.65f,
                    0.68f,
                    (float) (1 + (1 - cm.GetConnectedBoundingBox().maxZ)));
                break;
            case 3: // South, Inventory is to the north
                this.setBlockBounds(
                    0.35f,
                    0.32f,
                    (float) (0 - cm.GetConnectedBoundingBox().minZ),
                    0.65f,
                    0.68f,
            ...
            case 4: // West, Inventory is to the east
                this.setBlockBounds(
                    (float) (1 + cm.GetConnectedBoundingBox().minX) - 0.1f,
                    0.32f,
                    0.35f,
                    (float) (1 + cm.GetConnectedBoundingBox().minX),
                    0.68f,
                    0.65f);
                break;
            case 5: // East, Inventory is to the west
                this.setBlockBounds(
                    (float) (-1 * (1 - cm.GetConnectedBoundingBox().maxX)),
                    0.32f,
                    0.35f,
                    (float) (-1 * (1 - cm.GetConnectedBoundingBox().maxX)) + 0.1f,
                    0.68f,
                    0.65f);
                break;
        }
    ...
    public static boolean isOutputMarker(int operandType) {
        return operandType == META_OUT || operandType == META_LIKE_EXPORT
    ...
    public static boolean isInputMarker(int operandType) {
        return operandType == META_IN || operandType == META_REGULATE_MULTI
