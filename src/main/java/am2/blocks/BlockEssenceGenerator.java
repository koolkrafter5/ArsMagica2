...
public class BlockEssenceGenerator extends AMSpecialRenderPoweredBlock {
    ...
    public ArrayList<ItemStack> getDrops(World arg0, int arg1, int arg2, int arg3, int arg4, int arg5) {
        ...
        if (this == BlocksCommonProxy.obelisk) drops.add(new ItemStack(BlocksCommonProxy.obelisk));
        else if (this == BlocksCommonProxy.blackAurem) drops.add(new ItemStack(BlocksCommonProxy.blackAurem));
        else if (this == BlocksCommonProxy.celestialPrism) drops.add(new ItemStack(BlocksCommonProxy.celestialPrism));
        return drops;
    }
    ...
    public void addCollisionBoxesToList(World par1World, int par2, int par3, int par4, AxisAlignedBB par5AxisAlignedBB,
        ...
        if (this == BlocksCommonProxy.blackAurem) return;
        super.addCollisionBoxesToList(par1World, par2, par3, par4, par5AxisAlignedBB, par6List, par7Entity);
    }

    @Override
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World par1World, int par2, int par3, int par4) {
        if (this == BlocksCommonProxy.blackAurem) return null;
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int face, float interactX,
        ...
        if (HandleSpecialItems(world, player, x, y, z)) return true;
    ...
    public TileEntity createNewTileEntity(World par1World, int i) {
        if (this.NexusType == NEXUS_DARK) return new TileEntityBlackAurem();
        else if (this.NexusType == NEXUS_LIGHT) return new TileEntityCelestialPrism();
        else return new TileEntityObelisk();
    }
