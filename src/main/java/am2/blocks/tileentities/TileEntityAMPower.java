    ...
    public void updateEntity() {
        if (!worldObj.isRemote && this.canRequestPower() && tickCounter++ >= getRequestInterval()) {
            ...
            for (PowerTypes type : powerTypes) {
                float amtObtained = PowerNodeRegistry.For(worldObj)
                    .requestPower(this, type, this.getChargeRate());
                if (amtObtained > 0) PowerNodeRegistry.For(worldObj)
                    .insertPower(this, type, amtObtained);
            }
        }
    }
