...
public class TileEntitySummoner extends TileEntityAMPower implements IInventory, IKeystoneLockable {
    ...
    private static final int powerPadding = 500; // extra power to charge before summoning so that it can be maintained
                                                 // for a while
        ...
        if (!worldObj.isRemote) {
            ...
            if (isRedstonePowered() && inventory[SUMMON_SLOT] != null) {
                ...
                if (ent != null) {
                    unsummonCreature();
                    PowerNodeRegistry.For(this.worldObj)
                        .insertPower(this, PowerTypes.NEUTRAL, summonCost / 2);
                }
            }
        }
    }
    ...
    private void summonCreature() {
        ...
        EntityLiving summon = ((Summon) SkillManager.instance.getSkill("Summon"))
            .summonCreature(inventory[SUMMON_SLOT], dummyCaster, dummyCaster, worldObj, xCoord, yCoord, zCoord);
        if (summon != null) {
