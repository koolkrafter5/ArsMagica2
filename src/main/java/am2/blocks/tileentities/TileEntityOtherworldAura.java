...
public class TileEntityOtherworldAura extends TileEntityAMPower {
    ...
    private String placedByUsername = ""; // username of the player who placed the block (used to get the shadow's
                                          // texture)
    ...
    private void spawnHelper() {
        if (helper != null || worldObj.isRemote) return;

        this.helper = new EntityShadowHelper(worldObj);
        this.helper.setPosition(xCoord, yCoord + 1, zCoord);
        if (this.watchTarget != null) this.helper.setAltarTarget(watchTarget);
        this.worldObj.spawnEntityInWorld(helper);

        if (this.watchTarget != null) {
            this.helper
                .setDropoffLocation(new AMVector3(watchTarget.xCoord, watchTarget.yCoord - 2, watchTarget.zCoord));
            if (placedByUsername != null && !placedByUsername.isEmpty()) this.helper.setMimicUser(placedByUsername);
        }
    }
            ...
            if (PowerNodeRegistry.For(worldObj)
                ...
                if (delayCounter-- <= 0) {
                    ...
                    if (next.getItem() == ItemsCommonProxy.essence
                        && next.getItemDamage() > ItemsCommonProxy.essence.META_MAX) {
                        ...
                        if (location == null) {
                            ...
                            cantFindItem = false;
                            if (!this.helper.hasSearchLocation()) this.helper.setSearchLocationAndItem(location, next);
                ...
                if (this.helper != null) this.helper.unSummon();
            }
        }
    }
