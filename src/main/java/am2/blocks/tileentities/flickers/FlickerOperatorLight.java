...
public class FlickerOperatorLight implements IFlickerFunctionality {
    ...
    public void RemoveOperator(World worldObj, IFlickerController habitat, boolean powered) {
        ...
        if (!worldObj.isRemote) {
            ...
            for (int i = ((TileEntity) habitat).xCoord - radius; i <= ((TileEntity) habitat).xCoord + radius; ++i) {
                for (int j = ((TileEntity) habitat).yCoord - yRadius; j
                    <= ((TileEntity) habitat).yCoord + yRadius; ++j) {
                    for (int k = ((TileEntity) habitat).zCoord - radius; k
                        <= ((TileEntity) habitat).zCoord + radius; ++k) {
    ...
    public Object[] getRecipe() {
        return new Object[] { "ISI", "F L", "ISI", Character.valueOf('F'),
            new ItemStack(ItemsCommonProxy.flickerJar, 1, Affinity.FIRE.ordinal()), Character.valueOf('S'),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_SUNSTONE), Character.valueOf('L'),
            new ItemStack(ItemsCommonProxy.flickerJar, 1, Affinity.LIGHTNING.ordinal()), Character.valueOf('I'),
            ItemsCommonProxy.liquidEssenceBottle
