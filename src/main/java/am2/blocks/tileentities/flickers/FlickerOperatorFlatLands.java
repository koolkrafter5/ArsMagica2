...
public class FlickerOperatorFlatLands implements IFlickerFunctionality {
    ...
    public boolean DoOperation(World worldObj, IFlickerController habitat, boolean powered) {
        ...
        if (!worldObj.isRemote) {
            ...
            for (int i = 0; i < searchesPerLoop && !actionPerformed; ++i) {
                ...
                if (effectX == ((TileEntity) habitat).xCoord && effectY == ((TileEntity) habitat).yCoord
                    && effectZ == ((TileEntity) habitat).zCoord) return false;

                Block block = worldObj.getBlock(effectX, effectY, effectZ);
                int meta = worldObj.getBlockMetadata(effectX, effectY, effectZ);

                if (block != null && !worldObj.isAirBlock(effectX, effectY, effectZ)
                    && block.isOpaqueCube()
                    && block != BlocksCommonProxy.invisibleUtility) {
                    if (ForgeEventFactory.doPlayerHarvestCheck(new DummyEntityPlayer(worldObj), block, true)) {
                        if (block
                            .removedByPlayer(worldObj, new DummyEntityPlayer(worldObj), effectX, effectY, effectZ)) {
                            ...
                            if (!worldObj.isRemote) worldObj.playAuxSFX(
                                2001,
                                effectX,
                                effectY,
                                effectZ,
                                Block.getIdFromBlock(block)
                                    + (worldObj.getBlockMetadata(effectX, effectY, effectZ) << 12));
    ...
    public Object[] getRecipe() {
        return new Object[] { "S P", "ENI", " R ", Character.valueOf('S'), Items.iron_shovel, Character.valueOf('P'),
            ...
            new ItemStack(ItemsCommonProxy.flickerJar, 1, Affinity.EARTH.ordinal()), Character.valueOf('N'),
            new ItemStack(ItemsCommonProxy.essence, 1, ItemsCommonProxy.essence.META_EARTH), Character.valueOf('I'),
            new ItemStack(ItemsCommonProxy.flickerJar, 1, Affinity.ICE.ordinal()), Character.valueOf('R'),
