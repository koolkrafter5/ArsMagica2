...
public class FlickerOperatorItemTransport implements IFlickerFunctionality {
    ...
    public boolean DoOperation(World worldObj, IFlickerController controller, boolean powered, Affinity[] flickers) {
        ...
        for (int inIterator = habitat.getInListPosition(); inIterator < habitat.getInListSize(); ++inIterator) {
            ...
            TileEntity te = null;
            TileEntityCrystalMarker crystalMarkerTE = GetCrystalMarkerTileEntity(
                worldObj,
                (int) vector.x,
                (int) vector.y,
                (int) vector.z);
    ...
    private boolean moveItem(World worldObj, IInventory inventory, TileEntityCrystalMarker crystalMarkerTE,
        ...
        int currentSlot = 0;
        GetFirstStackStartingFromSlotResult result = InventoryUtilities
            .getFirstStackStartingFromSlot(inventory, orgStack, currentSlot, crystalMarkerTE.getFacing());
        ...
        while (orgStack != null) {
            if (orgStack != null && InputCanMove(crystalMarkerTE, orgStack, inventory)) {
                ...
                if (FindOutput(worldObj, habitat, mergeStack, inventory)) {
                    // if a valid item was found in input, and a valid output for it was found remove the item from the
                    // source
        ...
        for (int priority = 0; priority <= TileEntityFlickerHabitat.PRIORITY_FINAL; ++priority) {
            ...
            while (!fullLoop) {
                ...
                if (crystalMarkerTE == null) {
                    ...
                    removeFromOutList.get(priority)
                        .add(vector);
                    break;
                }
                ...
                if (te != null && te instanceof IInventory) {
                    ...
                    itemMoved = outputItem(
                        markerType,
                        new IInventory[] { inventory },
                        stack,
                        crystalMarkerTE,
                ...
                if (!itemMoved && te instanceof TileEntityChest) {
                    ...
                    if (adjacent != null) {
                        ...
                        itemMoved = outputItem(
                            markerType,
                            new IInventory[] { inventory, (IInventory) te },
                            stack,
                            crystalMarkerTE,
                            new IInventory[] { source });
    ...
    private boolean outputItem(int markerType, IInventory[] outputs, ItemStack stack,
        ...
        switch (markerType) {
            ...
            case BlockCrystalMarker.META_LIKE_EXPORT:
                // One of the destination inventories must already contain at least 1 of the item we are trying to move
                // and
            ...
            case BlockCrystalMarker.META_REGULATE_EXPORT:
                // Check that the item exists in the item filter and that the inventory doesn't have more then the
                // allocated amount already
                // and that the inventory has room for the item
                for (IInventory inventory : outputs) {
                    if (crystalMarkerTE.filterHasItem(stack)
                        && InventoryUtilities.getLikeItemCount(inventory, stack, side)
                            < crystalMarkerTE.getFilterCount(stack)
                        && InventoryUtilities.inventoryHasRoomFor(inventory, stack, 1, side)
                        && isDestinationDifferentFromSource(outputs, sources)) {
                        return InventoryUtilities.mergeIntoInventory(inventory, stack, stack.stackSize, side);
                    }
    ...
    public Object[] getRecipe() {
        return new Object[] { " B ", "CAC", " B ", Character.valueOf('A'),
            new ItemStack(ItemsCommonProxy.flickerJar, 1, Affinity.AIR.ordinal()), Character.valueOf('C'),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_CHIMERITE), Character.valueOf('B'),
