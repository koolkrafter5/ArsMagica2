...
public class FlickerOperatorFelledOak implements IFlickerFunctionality {
    ...
    void beginTreeFelling(World world, int x, int y, int z) {
        ...
        if (wood.isWood(world, x, y, z)) {
            ...
            if (numLeaves > 3) destroyTree(world, x, y, z, wood, meta);
    ...
    private void plantTree(World worldObj, IFlickerController habitat, boolean powered) {
        if (!powered || worldObj.isRemote) return;
        ...
        worldObj.setBlock(
            ...
            block.getMetadata(sapling.getItemDamage()),
            3);
    }

    private AMVector3 getPlantLocation(World worldObj, IFlickerController habitat, ItemStack sapling) {
        if (sapling.getItem() instanceof ItemBlock == false) return null;
        ...
        AMDataWriter writer = new AMDataWriter();
        writer.add(te.xCoord - radius_horiz)
            .add(te.zCoord - radius_horiz);
        habitat.setMetadata(this, writer.generate());
    ...
    private ItemStack getSaplingFromNearbyChest(World worldObj, IFlickerController habitat) {
        for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            ...
            int index = InventoryUtilities
                .getInventorySlotIndexFor(inv, new ItemStack(Blocks.sapling, 1, Short.MAX_VALUE));
            if (index > -1) {
    ...
    /**
     * Gets an instance of the adjacent IInventory at direction offset. Returns null if not found or invalid type
     * adjacent.
     */
    private IInventory getOffsetInventory(World worldObj, IFlickerController habitat, ForgeDirection direction) {
        TileEntity te = (TileEntity) habitat;
        TileEntity adjacent = worldObj
            .getTileEntity(te.xCoord + direction.offsetX, te.yCoord + direction.offsetY, te.zCoord + direction.offsetZ);
        if (adjacent != null && adjacent instanceof IInventory) return (IInventory) adjacent;
        return null;
    }
        ...
        for (int i = -radius; i <= radius; ++i) {
            for (int j = -radius; j <= radius; ++j) {
                Block block = worldObj.getBlock(
                    ((TileEntity) habitat).xCoord + i,
                    ((TileEntity) habitat).yCoord,
                    ((TileEntity) habitat).zCoord + j);
                ...
                if (block.isWood(
                    ...
                    ((TileEntity) habitat).yCoord,
                    ((TileEntity) habitat).zCoord + j)) {
                    if (!worldObj.isRemote) beginTreeFelling(
                        worldObj,
                        ((TileEntity) habitat).xCoord + i,
                        ((TileEntity) habitat).yCoord,
                        ((TileEntity) habitat).zCoord + j);
    ...
    public int TimeBetweenOperation(boolean powered, Affinity[] flickers) {
        ...
        float augments = 1.0f;
        for (Affinity aff : flickers) {
            if (aff == Affinity.LIGHTNING) augments *= 0.5f;
        }
    ...
    public Object[] getRecipe() {
        return new Object[] { "WG ", "NCL", " OW", Character.valueOf('W'), BlocksCommonProxy.witchwoodLog,
            ...
            Character.valueOf('G'),
            new ItemStack(ItemsCommonProxy.bindingCatalyst, 1, ItemsCommonProxy.bindingCatalyst.META_AXE) };
    }
}
