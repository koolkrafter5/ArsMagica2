...
public class FlickerOperatorProgeny implements IFlickerFunctionality {
    ...
    public boolean DoOperation(World worldObj, IFlickerController habitat, boolean powered) {
        ...
        for (EntityAnimal creature : creatures) {
            ...
            if (!creature.isInLove() && creature.getGrowingAge() == 0) count++;
            ...
            if (count == 2) {
                if (worldObj.isRemote) {
                    ...
                    creatures = worldObj.getEntitiesWithinAABB(
                        clazz,
                        AxisAlignedBB.getBoundingBox(
                            ((TileEntity) habitat).xCoord - radius,
                            ((TileEntity) habitat).yCoord - radius,
    ...
    public int TimeBetweenOperation(boolean powered, Affinity[] flickers) {
        if (powered) {
            ...
            for (Affinity aff : flickers) if (aff == Affinity.LIGHTNING) base *= COOLDOWN_BONUS;
    ...
    public Object[] getRecipe() {
        return new Object[] { "ELE", "EFE", "EWE", Character.valueOf('E'), Items.egg, Character.valueOf('L'),
            new ItemStack(ItemsCommonProxy.essence, 1, ItemsCommonProxy.essence.META_LIFE), Character.valueOf('F'),
            new ItemStack(ItemsCommonProxy.flickerJar, 1, Affinity.LIFE.ordinal()), Character.valueOf('W'),
