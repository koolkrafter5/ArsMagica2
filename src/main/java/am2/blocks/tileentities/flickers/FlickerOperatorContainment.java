...
public class FlickerOperatorContainment implements IFlickerFunctionality {
    ...
    protected void setUtilityBlock(World world, int x, int y, int z, int meta) {
        ...
        if (world.getBlock(x, y, z) == BlocksCommonProxy.invisibleUtility) {
            ...
            if ((meta == 3 && exMeta == 4) || (meta == 4 && exMeta == 3)) meta = 7;
            else if ((meta == 5 && exMeta == 6) || (meta == 6 && exMeta == 5)) meta = 8;

            world.setBlock(x, y, z, BlocksCommonProxy.invisibleUtility, meta, 2);
        } else {
            if (world.isAirBlock(x, y, z)) world.setBlock(x, y, z, BlocksCommonProxy.invisibleUtility, meta, 2);
        }
    }
    ...
    public boolean DoOperation(World worldObj, IFlickerController habitat, boolean powered, Affinity[] flickers) {
        ...
        for (int i = 0; i < calcRadius * 2 + 1; ++i) {
            ...
            if (hasArcaneAugment) {
                ...
                setUtilityBlock(
                    worldObj,
                    ((TileEntity) habitat).xCoord - calcRadius,
                    ((TileEntity) habitat).yCoord,
                    ((TileEntity) habitat).zCoord + calcRadius + 1 - i,
                    9);
                // +x
                setUtilityBlock(
                    worldObj,
                    ((TileEntity) habitat).xCoord + calcRadius + 1,
                    ((TileEntity) habitat).yCoord,
                    ((TileEntity) habitat).zCoord - calcRadius + i,
                    9);
                // -z
                setUtilityBlock(
                    worldObj,
                    ((TileEntity) habitat).xCoord - calcRadius + i,
                    ((TileEntity) habitat).yCoord,
                    ((TileEntity) habitat).zCoord - calcRadius,
                    9);
                // +z
                setUtilityBlock(
                    worldObj,
                    ((TileEntity) habitat).xCoord + calcRadius + 1 - i,
                    ((TileEntity) habitat).yCoord,
                    ((TileEntity) habitat).zCoord + calcRadius + 1,
                    9);
            } else {
                // -x
                setUtilityBlock(
    ...
    public void RemoveOperator(World worldObj, IFlickerController habitat, boolean powered) {
        ...
        for (int i = 0; i < radius * 2 + 1; ++i) {
            ...
            clearUtilityBlock(
                ...
                ((TileEntity) habitat).yCoord,
                ((TileEntity) habitat).zCoord + radius + 1 - i);
            // +x
            clearUtilityBlock(
                worldObj,
                ((TileEntity) habitat).xCoord + radius + 1,
                ((TileEntity) habitat).yCoord,
                ((TileEntity) habitat).zCoord - radius + i);
            // -z
            clearUtilityBlock(
                worldObj,
                ((TileEntity) habitat).xCoord - radius + i,
                ((TileEntity) habitat).yCoord,
                ((TileEntity) habitat).zCoord - radius);
            // +z
            clearUtilityBlock(
                worldObj,
                ((TileEntity) habitat).xCoord + radius + 1 - i,
                ((TileEntity) habitat).yCoord,
    ...
    public Object[] getRecipe() {
        return new Object[] { "FWF", "ARN", "IWI", Character.valueOf('F'), Blocks.fence, Character.valueOf('W'),
            Blocks.cobblestone_wall, Character.valueOf('A'),
            new ItemStack(ItemsCommonProxy.flickerJar, 1, Affinity.AIR.ordinal()), Character.valueOf('R'),
            new ItemStack(ItemsCommonProxy.rune, 1, ItemsCommonProxy.rune.META_BLUE), Character.valueOf('N'),
            new ItemStack(ItemsCommonProxy.flickerJar, 1, Affinity.ENDER.ordinal()), Character.valueOf('I'),
            Blocks.iron_bars
