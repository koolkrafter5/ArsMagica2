...
public class TileEntityFlickerControllerBase extends TileEntityAMPower implements IFlickerController {
    ...
    protected void setOperator(IFlickerFunctionality operator) {
        if (this.operator != null) {
            this.operator.RemoveOperator(
                worldObj,
                this,
                PowerNodeRegistry.For(worldObj)
                    .checkPower(this, this.operator.PowerPerOperation()),
                nearbyList);
        }
    ...
    public void updateOperator(ItemStack stack) {
        if (stack == null || stack.getItem() != ItemsCommonProxy.flickerFocus) return;
    ...
    public void updateEntity() {
        ...
        if (operator != null) {
            boolean powered = PowerNodeRegistry.For(worldObj)
                .checkPower(this, operator.PowerPerOperation());
            ...
            if (tickCounter++ >= operator.TimeBetweenOperation(powered, unpoweredNeighbors)) {
                ...
                if ((powered && operator.RequiresPower()) || !operator.RequiresPower()) {
                    ...
                    if (success || operator.RequiresPower()) PowerNodeRegistry.For(worldObj)
                        .consumePower(
                            this,
                            PowerNodeRegistry.For(worldObj)
    ...
    private Affinity[] getUnpoweredNeighbors() {
        ...
        for (int i = 0; i < nearbyList.length; ++i) {
            ...
            if (nearbyList[i] == null || worldObj
                .isBlockIndirectlyGettingPowered(xCoord + dir.offsetX, yCoord + dir.offsetY, zCoord + dir.offsetZ)) {
                aff[i] = null;
            } else {
                aff[i] = nearbyList[i];
            }
    ...
    public void readFromNBT(NBTTagCompound par1nbtTagCompound) {
        ...
        NBTTagList sigilMetaStore = par1nbtTagCompound
            .getTagList("sigil_metadata_collection", Constants.NBT.TAG_COMPOUND);
