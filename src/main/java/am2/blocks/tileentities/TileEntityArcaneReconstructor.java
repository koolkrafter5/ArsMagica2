...
public class TileEntityArcaneReconstructor extends TileEntityAMPower
    ...
    public void updateEntity() {
        if (isFirstTick) {
            outerRingRotationSpeeds = new AMVector3(
                worldObj.rand.nextDouble() * 4 - 2,
                worldObj.rand.nextDouble() * 4 - 2,
                worldObj.rand.nextDouble() * 4 - 2);
            middleRingRotationSpeeds = new AMVector3(
                worldObj.rand.nextDouble() * 4 - 2,
                worldObj.rand.nextDouble() * 4 - 2,
                worldObj.rand.nextDouble() * 4 - 2);
            innerRingRotationSpeeds = new AMVector3(
                worldObj.rand.nextDouble() * 4 - 2,
                worldObj.rand.nextDouble() * 4 - 2,
                worldObj.rand.nextDouble() * 4 - 2);
            isFirstTick = false;
        ...
        if (PowerNodeRegistry.For(this.worldObj)
            .checkPower(this, this.getRepairCost())) {// has enough power
            if ((repairCounter++ % getRepairRate() == 0) && (!queueRepairableItem())) {// has ticked and already has
                                                                                       // item queued
        ...
        if (worldObj.isRemote) {
            ...
            if (shouldRenderItemStack()) {
                AMParticle p = (AMParticle) AMCore.instance.proxy.particleManager.spawn(
                    worldObj,
                    "sparkle2",
                ...
                if (p != null) {
                    ...
                    p.AddParticleController(
                        new ParticleFadeOut(p, 1, false).setFadeSpeed(0.035f)
    ...
    private EntityLiving getDummyEntity() {
        if (dummyEntity == null) dummyEntity = new EntityDummyCaster(this.worldObj);
        return dummyEntity;
    }
    ...
    private boolean performRepair() {
        ...
        if (inventory[SLOT_ACTIVE].isItemDamaged()) {
            if (!worldObj.isRemote) inventory[SLOT_ACTIVE].damageItem(-1, getDummyEntity());
            return true;
        } else {
