...
public class TileEntityEverstone extends TileEntity {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                for (int k = -1; k <= 1; k++) {
                    ...
                    Block blockID = worldObj
                        .getBlock(targetPosition.chunkPosX, targetPosition.chunkPosY, targetPosition.chunkPosZ);
                    if (blockID == BlocksCommonProxy.everstone && !completedUpdates.contains(targetPosition)) {
                        TileEntityEverstone everstone = ((TileEntityEverstone) worldObj.getTileEntity(
                            targetPosition.chunkPosX,
                            targetPosition.chunkPosY,
                            targetPosition.chunkPosZ));
                        if (everstone != null) everstone.propagatePoweredByEverstone(powered, completedUpdates);
                    }
                }
            }
        }
    ...
    public void updateEntity() {
        ...
        if (reconstructTimer <= 0) return;

        if (!worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord) && !poweredFromEverstone) {
            reconstructTimer--;
            if (worldObj.isRemote) { //
                // worldObj.scheduleBlockUpdateWithPriority(xCoord, yCoord, zCoord, BlocksCommonProxy.everstone.blockID,
                // 0, 0);
                worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
                if (reconstructTimer < reconstructMax - 20 && reconstructTimer > 20 && worldObj.rand.nextInt(10) < 8) {
                    AMCore.proxy.addDigParticle(
                        worldObj,
                        xCoord,
                        yCoord,
                        zCoord,
                        getFacade() == null ? BlocksCommonProxy.everstone : getFacade(),
                        getFacadeMeta());
                }
            }
        }
    ...
    public int getFadeStrength() {
        if (reconstructTimer > reconstructMax / 2) return 0;
    ...
    public void writeToNBT(NBTTagCompound par1nbtTagCompound) {
        ...
        if (facade != null) {
            par1nbtTagCompound.setString(
                "facade",
                facade.getUnlocalizedName()
                    .replace("tile.", ""));
