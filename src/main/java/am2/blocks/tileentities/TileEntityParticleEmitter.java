...
public class TileEntityParticleEmitter extends TileEntity {
    ...
    public void updateEntity() {
        ...
        if (!show && worldObj.isRemote && ((forceShow && showTicks++ > 100) || !forceShow)) {
            ...
            if (localPlayer != null && localPlayer.inventory.getCurrentItem() != null
                && localPlayer.inventory.getCurrentItem()
    ...
    private void doSpawn() {
        ...
        AMParticle particle = (AMParticle) AMCore.proxy.particleManager
            .spawn(worldObj, AMParticle.particleTypes[particleType], x, y, z);
        if (particle != null) {
            particle.AddParticleController(
                AMCore.proxy.particleManager.createDefaultParticleController(
                    particleBehaviour,
                    particle,
                    new AMVector3(x, y, z),
                    speed,
            ...
            if (!defaultColor) {
                if (!randomColor) particle.setRGBColorF(
                    ((particleColor >> 16) & 0xFF) / 255f,
                    ((particleColor >> 8) & 0xFF) / 255f,
                    (particleColor & 0xFF) / 255f);
                else particle
                    .setRGBColorF(worldObj.rand.nextFloat(), worldObj.rand.nextFloat(), worldObj.rand.nextFloat());
            }
        }
    }
