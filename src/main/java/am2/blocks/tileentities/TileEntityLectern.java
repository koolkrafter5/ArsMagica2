...
public class TileEntityLectern extends TileEntityEnchantmentTable {
        ...
        if (worldObj.isRemote) {
            ...
            if (tooltipStack != null && field_145926_a % 2 == 0) {
                AMParticle particle = (AMParticle) AMCore.proxy.particleManager.spawn(
                    worldObj,
                    "sparkle",
                    xCoord + 0.5 + ((worldObj.rand.nextDouble() * 0.2) - 0.1),
                    yCoord + 1,
                    zCoord + 0.5 + ((worldObj.rand.nextDouble() * 0.2) - 0.1));
                if (particle != null) {
                    particle.AddParticleController(
                        new ParticleMoveOnHeading(
                            particle,
                            worldObj.rand.nextDouble() * 360,
                            -45 - worldObj.rand.nextInt(90),
                            0.05f,
                            1,
                            false));
                    particle.AddParticleController(
                        new ParticleFadeOut(particle, 2, false).setFadeSpeed(0.05f)
    ...
    private void updateBookRender() {
        ...
        if (increasing) {
            particleMaxAge += 2;
            if (particleMaxAge - particleAge > 120) increasing = false;
        } else {
            if (particleMaxAge - particleAge < 5) increasing = true;
        }
