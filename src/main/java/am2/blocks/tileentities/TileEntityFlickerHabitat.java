...
public class TileEntityFlickerHabitat extends TileEntityFlickerControllerBase implements IInventory {
    ...
    private void initLocationLists() {
        ...
        for (int i = 0; i < PRIORITY_LEVELS; ++i) outList.put(i, new ArrayList<AMVector3>());
    }
    ...
    public int getOutListPosition(int priority) {
        if (!outListPositions.containsKey(priority)) outListPositions.put(priority, 0);
    ...
    public AMVector3 getOutListAt(int priority, int index) {
        if (outList != null && outList.containsKey(priority)
            && outList.get(priority)
                .size() > 0)
            return outList.get(priority)
                .get(index);
        return new AMVector3(this);
    }

    public void setOutListAt(int priority, int index, AMVector3 value) {
        if (outList.containsKey(priority)) this.outList.get(priority)
            .set(index, value);
    }

    public void removeOutListAt(int priority, int index) {
        if (this.outList.containsKey(priority)) this.outList.get(priority)
            .remove(index);
    }

    public void removeOutListAt(int priority, AMVector3 value) {
        if (!this.outList.containsKey(priority)) return;
        this.outList.get(priority)
            .remove(value);
    }

    public int getOutListSize(int priority) {
        if (this.outList.containsKey(priority)) return this.outList.get(priority)
            .size();
        return 0;
    }
    ...
    public void AddMarkerLocationOut(AMVector3 markerLocation) {
        ...
        if (out != BlocksCommonProxy.crystalMarker) return;

        TileEntity te = this.worldObj
            .getTileEntity((int) markerLocation.x, (int) markerLocation.y, (int) markerLocation.z);
        if (te == null || te instanceof TileEntityCrystalMarker == false) return;

        int priority = ((TileEntityCrystalMarker) te).getPriority();

        if (!outList.containsKey(priority)) outList.put(priority, new ArrayList<AMVector3>());

        if (!outList.get(priority)
    ...
    private void readOutList(NBTTagCompound compound) {
        // valid compound?
        if (!compound.hasKey("outList")) return;
    ...
    public int getCrystalColor() {
        if (this.flickerJar == null) return 0;
        ...
        if (this.flickerJar.getItem() == ItemsCommonProxy.flickerJar)
            ...
            for (Affinity aff : Affinity.values()) {
                ...
                if ((meta & aff.getAffinityMask()) == aff.getAffinityMask()) affinities.add(aff);
            }
            ...
            if (affinities.size() > 0) {
                ...
                if (this.fadeCounter > this.MAX_SHIFT_TICKS) return secondColor;
    ...
    private void setUpgradeOfMainHabitat() {
        if (this.mainHabitatDirection != ForgeDirection.UNKNOWN) {
            TileEntity te = worldObj.getTileEntity(
                this.xCoord + this.mainHabitatDirection.offsetX,
                this.yCoord + this.mainHabitatDirection.offsetY,
                this.zCoord + this.mainHabitatDirection.offsetZ);
    ...
    public void switchMarkerPriority(AMVector3 vec, int oldPriority, int priority) {
        if (this.outList.containsKey(oldPriority)) this.removeOutListAt(oldPriority, vec);

        if (!this.outList.containsKey(priority)) this.outList.put(priority, new ArrayList<AMVector3>());
