...
public class TileEntityCrystalMarkerSpellExport extends TileEntityCrystalMarker {
    ...
    private void spawnParticles() {
        for (int i = 0; i < 15; ++i) {
            AMParticle effect = (AMParticle) AMCore.proxy.particleManager
                .spawn(worldObj, "sparkle2", xCoord, yCoord, zCoord);
            if (effect != null) {
                effect.AddParticleController(
                    new ParticleFloatUpward(effect, 0, worldObj.rand.nextFloat() * 0.1f, 1, false));
                effect.AddParticleController(
                    new ParticleFadeOut(effect, 2, false).setFadeSpeed(0.035f)
    ...
    private TileEntityCraftingAltar getCATE(AMVector3 vec) {
        ...
        if (te != null && te instanceof TileEntityCraftingAltar) return (TileEntityCraftingAltar) te;
