...
public class TileEntityCraftingAltar extends TileEntityAMPower implements IMultiblockStructureController {
    ...
    private void setupMultiblock() {
        ...
        Block[] augMatls = new Block[] { Blocks.glass, Blocks.coal_block, Blocks.redstone_block, Blocks.iron_block,
            Blocks.lapis_block, Blocks.gold_block, Blocks.diamond_block, Blocks.emerald_block, BlocksCommonProxy.AMOres,
            BlocksCommonProxy.AMOres };

        int[] augMetas = new int[] { 0, // glass
            0, // coal
            0, // redstone
            0, // iron
            0, // lapis
            ...
            0, // diamond
            0, // emerald
            BlocksCommonProxy.AMOres.META_MOONSTONE_BLOCK, BlocksCommonProxy.AMOres.META_SUNSTONE_BLOCK };
    ...
    private int getNumPartsInSpell() {
        ...
        if (shapeGroupGuide != null) {
            for (int i = 0; i < shapeGroupGuide.length; ++i) {
                if (shapeGroupGuide[i] != null) parts += shapeGroupGuide[i].length;
            }
        }
        return parts;
    }
    ...
    private boolean matchCurrentRecipe() {
        ...
        if (part instanceof ISpellModifier) {
            metaData = ((ISpellModifier) part)
                .getModifierMetadata(currentAddedItems.toArray(new ItemStack[currentAddedItems.size()]));
            if (metaData == null) {
                metaData = new byte[0];
            }
        }
        ...
        if (primaryvalid || secondaryvalid) {
            ...
            if (mainmatlGroups != null && mainmatlGroups.size() == 1) {
                ...
                if (group == wood_primary || group == wood_secondary) {
                    maxEffects += 1;
                } else if (group == cobble_primary || group == cobble_secondary
                    || group == sandstone_primary
                    || group == sandstone_secondary) {
                        maxEffects += 1;
                    } else if (group == brick_primary || group == brick_secondary
                        || group == witchwood_primary
                        || group == witchwood_secondary) {
                            maxEffects += 2;
                        } else if (group == netherbrick_primary || group == netherbrick_secondary
                            || group == quartz_primary
                            || group == quartz_secondary) {
                                maxEffects += 3;
                                stability += 1F;
    ...
    private void checkForStartCondition() {
        ...
        if (items.size() == 1) {
            ...
            if (item != null && !item.isDead
                ...
                && item.getEntityItem()
                    .getItemDamage() == ItemsCommonProxy.rune.META_BLANK) {
                item.setDead();
                setCrafting(true);
            }
        }
        ...
        if (items.size() == 1) {
            ...
            if (item != null && !item.isDead
                ...
                if (!worldObj.isRemote) {
                    ...
                    craftStack.stackTagCompound
                        .setString("suggestedName", currentSpellName != null ? currentSpellName : "");
        ...
        if (!worldObj.isRemote) {
            ...
            for (ItemStack stack : allAddedItems) {
                if (stack.getItem() == ItemsCommonProxy.essence
                    && stack.getItemDamage() > ItemsCommonProxy.essence.META_MAX) continue;
