...
public class TileEntitySlipstreamGenerator extends TileEntityAMPower {
    ...
    public void updateEntity() {
        ...
        while (it.hasNext()) {
            ...
            if (PowerNodeRegistry.For(this.worldObj)
                ...
                if (worldObj.isRemote) spawnParticles(player);
    ...
    private void spawnParticles(EntityPlayer player) {
        AMParticle wind = (AMParticle) AMCore.proxy.particleManager
            .spawn(worldObj, "wind", player.posX, player.posY - player.height, player.posZ);
        float pitch = player.rotationPitch;
        float factor = (pitch > 0 ? (pitch - 10) : (pitch + 10)) / -180.0f;
        if (player.isSneaking()) factor = 0.01f;
        if (wind != null) {
            wind.setMaxAge(10);
    ...
    private void refreshPlayerList() {
        ...
        for (int i = 0; i < worldObj.playerEntities.size(); ++i) {
            ...
            if (playerIsValid(player) && !levitatingEntities.contains(player)) levitatingEntities.add(player);
        }
    }
