...
public class TileEntitySeerStone extends TileEntityAMPower implements IInventory, IKeystoneLockable {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote && isActive()) {
            if (hasSight) PowerNodeRegistry.For(worldObj)
                .consumePower(this, PowerTypes.LIGHT, 0.25f);
            else PowerNodeRegistry.For(worldObj)
                .consumePower(this, PowerTypes.LIGHT, 0.125f);
        }
        ...
        if (ticksToNextCheck <= 0 && isActive()) {
            ...
            if (searchClass != null) {
                nearbyMobs = (ArrayList<Entity>) this.worldObj.getEntitiesWithinAABB(
                    searchClass,
                ...
                if (key > 0) {
                    ...
                    for (Entity e : nearbyMobs) {
                        if (swapDetectionMode) {
                            ...
                            if (!KeystoneUtilities.instance.GetKeysInInvenory((EntityLivingBase) e)
                                .contains(key)) {
                                mobsToIgnore.add(e);
                            }
                        } else {
