...
public class TileEntityCalefactor extends TileEntityAMPower implements IInventory, ISidedInventory, IKeystoneLockable {
    ...
    private boolean canSmelt() {
        if (this.calefactorItemStacks[0] == null) {
            ...
            ItemStack var1 = FurnaceRecipes.smelting()
                .getSmeltingResult(this.calefactorItemStacks[0]);
    ...
    public void smeltItem() {
        if (this.canSmelt()) {
            ...
            if (this.calefactorItemStacks[0].getItem() instanceof ItemFood
                ...
                || this.calefactorItemStacks[0].getItem() == ItemsCommonProxy.itemOre) {
                if (PowerNodeRegistry.For(worldObj)
                    .checkPower(this, PowerTypes.DARK, getCookTickPowerCost()))
                    if (PowerNodeRegistry.For(worldObj)
                        .checkPower(this, PowerTypes.NEUTRAL, getCookTickPowerCost()))
                        if (PowerNodeRegistry.For(worldObj)
                            .checkPower(this, PowerTypes.LIGHT, getCookTickPowerCost())) smeltStack.stackSize++;
            }
            ...
            if (doSmelt) {
                ...
                if (Math.random() <= 0.25) {
                    if (calefactorItemStacks[5] == null) {
                        calefactorItemStacks[5] = new ItemStack(
                            ItemsCommonProxy.itemOre,
                            1,
                            ItemsCommonProxy.itemOre.META_VINTEUMDUST);
                    } else {
    ...
    protected void sendCookStatusUpdate(boolean isCooking) {
        ...
        if (this.calefactorItemStacks[0] != null) writer.add(this.calefactorItemStacks[0]);
    ...
    public void updateEntity() {
            ...
            if (this.isCooking) {
                ...
                if (particleCount <= 0) {
                    ...
                    AMCore.proxy.particleManager
                        .BeamFromPointToPoint(worldObj, rStartX, rStartY, rStartZ, endX, endY, endZ, 0xFF8811);
                    if (worldObj.rand.nextBoolean()) {
                        AMParticle effect = (AMParticle) AMCore.instance.proxy.particleManager
                            .spawn(worldObj, "smoke", endX, endY, endZ);
                        if (effect != null) {
                        ...
                        AMParticle effect = (AMParticle) AMCore.instance.proxy.particleManager
                            .spawn(worldObj, "explosion_2", endX, endY, endZ);
                        if (effect != null) {
                            effect.setIgnoreMaxAge(false);
                            effect.setMaxAge(10);
        ...
        boolean powerCheck = PowerNodeRegistry.For(this.worldObj)
            .checkPower(this, getCookTickPowerCost());
            ...
            if (!worldObj.isRemote) {
                if (PowerNodeRegistry.For(worldObj)
                    ...
                    && PowerNodeRegistry.For(worldObj)
                        .checkPower(this, PowerTypes.NEUTRAL, getCookTickPowerCost())
                    && PowerNodeRegistry.For(worldObj)
                        .checkPower(this, PowerTypes.LIGHT, getCookTickPowerCost())) {

                    PowerNodeRegistry.For(this.worldObj)
                        .consumePower(this, PowerTypes.DARK, getCookTickPowerCost());
                    PowerNodeRegistry.For(this.worldObj)
                        .consumePower(this, PowerTypes.NEUTRAL, getCookTickPowerCost());
                    PowerNodeRegistry.For(this.worldObj)
                        .consumePower(this, PowerTypes.LIGHT, getCookTickPowerCost());
