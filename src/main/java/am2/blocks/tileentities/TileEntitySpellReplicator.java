...
public class TileEntitySpellReplicator extends TileEntityAMPower implements IInventory {
    ...
    public void updateEntity() {
        ...
        if (worldObj.isRemote) {
            for (int i = 0; i < AMCore.config.getGFXLevel(); ++i) {
                particle = (AMParticle) AMCore.proxy.particleManager
                    .spawn(worldObj, isWorking() ? "symbols" : "ember", xCoord + 0.5f, yCoord + 0.5f, zCoord + 0.5f);
        ...
        if (this.inventory[0] != null && yetToConsume == -1) {
            ...
            if (!worldObj.isRemote) {
                float availablePower = PowerNodeRegistry.For(this.worldObj)
                    .getPower(this, PowerTypes.LIGHT);
                PowerNodeRegistry.For(this.worldObj)
                    .consumePower(this, PowerTypes.LIGHT, Math.min(yetToConsume, Math.min(availablePower, 100)));
    ...
    private float getTotalReplicateCost() {
        if (this.inventory[0] != null && this.inventory[0].getItem() instanceof SpellBase)
            return SpellUtils.instance.getSpellRequirements(
                this.inventory[0],
                this.worldObj.getClosestPlayer(this.xCoord, this.yCoord, this.zCoord, -1)).manaCost * 10;
        return -1;
    }
