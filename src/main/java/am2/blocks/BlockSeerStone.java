...
public class BlockSeerStone extends AMSpecialRenderPoweredBlock {
    ...
    public int onBlockPlaced(World world, int x, int y, int z, int side, float impX, float impY, float impZ, int meta) {
        ...
        if (side == 2 && world.getBlock(x, y, z + 1)
            .isSideSolid(world, x, y, z + 1, ForgeDirection.NORTH)) // -z
        {
            var10 = 3;
        }

        if (side == 3 && world.getBlock(x, y, z - 1)
            .isSideSolid(world, x, y, z - 1, ForgeDirection.SOUTH)) // +z
        {
            var10 = 4;
        }

        if (side == 4 && world.getBlock(x + 1, y, z)
            .isSideSolid(world, x + 1, y, z, ForgeDirection.WEST)) // -x
        {
            var10 = 5;
        }

        if (side == 5 && world.getBlock(x - 1, y, z)
            .isSideSolid(world, x - 1, y, z, ForgeDirection.EAST)) // +x
        {
            var10 = 6;
        }
    ...
    public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer,
        ...
        if (!par1World.isRemote) FMLNetworkHandler
            .openGui(par5EntityPlayer, AMCore.instance, ArsMagicaGuiIdList.GUI_SEER_STONE, par1World, par2, par3, par4);
        return true;
    }
    ...
    public int isProvidingStrongPower(IBlockAccess par1iBlockAccess, int x, int y, int z, int l) {
        ...
        if (myTE == null || !(myTE instanceof TileEntitySeerStone)) return 0;
