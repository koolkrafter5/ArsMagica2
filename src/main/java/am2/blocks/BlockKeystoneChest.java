...
public class BlockKeystoneChest extends AMSpecialRenderBlockContainer {
    ...
    public boolean onBlockActivated(World world, int par2, int par3, int par4, EntityPlayer entityplayer, int par6,
        ...
        if (!world.isRemote) {
            ...
            /*
             * PacketDispatcher.sendPacketToPlayer(AMCore.instance.proxy.packetSender.createArsMagicaClientPacket(
             * AMPacketIDs.KEYSTONE_CHEST_GUI_OPEN,
             * new AMDataWriter().add(par2).add(par3).add(par4).generate()), (Player)entityplayer);
             */

            FMLNetworkHandler
                .openGui(entityplayer, AMCore.instance, ArsMagicaGuiIdList.GUI_KEYSTONE_CHEST, world, par2, par3, par4);
