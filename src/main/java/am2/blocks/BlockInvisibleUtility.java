...
public class BlockInvisibleUtility extends AMBlock {
    ...
    public boolean canReplace(World worldIn, int p_149705_2_, int p_149705_3_, int p_149705_4_, int p_149705_5_,
        ItemStack stackIn) {
        return true;
    }
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World par1World, int par2, int par3, int par4) {
        ...
        if (meta == 0 || meta == 1 || meta == 2) return null;
    ...
    public void addCollisionBoxesToList(World world, int x, int y, int z, AxisAlignedBB axisAlignedBB,
        List collisionList, Entity entity) {

        if (entity == null || world == null || entity instanceof EntityPlayer || entity instanceof EntityBroom) return;
        ...
        if (meta > 2 && meta < 10) {
            ...
            if (world.isRemote && isCollided) spawnBlockParticles(world, x, y, z);
        }
    }
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        if (world.isRemote) {
            int meta = world.getBlockMetadata(x, y, z);
            if (meta > 2 && meta < 10) spawnBlockParticles(world, x, y, z);
        }
    }
    ...
    private void spawnBlockParticles(World world, int x, int y, int z) {
        AMParticle particle = (AMParticle) AMCore.proxy.particleManager
            .spawn(world, "symbols", x + 0.5, y + 0.5, z + 0.5);
        if (particle != null) {
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        ...
        if (meta < 3) {
            ...
            if (!buffNearby && world.getBlock(x, y, z) == this) world.setBlockToAir(x, y, z);
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        if (world.rand.nextInt(10) < 3 && world.isRemote && meta >= 3 && meta < 10) {
            List<Entity> ents = world.getEntitiesWithinAABB(
                Entity.class,
                AxisAlignedBB.getBoundingBox(x - 0.2, y - 0.2, z - 0.2, x + 1.2, y + 1.2, z + 1.2));
            if (ents.size() > 0) {
