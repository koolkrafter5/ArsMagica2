...
public class ArcaneReconstructorRenderer extends TileEntitySpecialRenderer {
    ...
    public ArcaneReconstructorRenderer() {
        ...
        rLoc = new ResourceLocation(
            "arsmagica2",
            ResourceManager.getCustomBlockTexturePath("Arcane_Reconstructor.png"));
    ...
    private void doRender(TileEntityArcaneReconstructor te, double d, double d1, double d2, float f) {
        ...
        if ((d != 0 || d1 != 0 || d2 != 0) && te.shouldRenderItemStack()) {
            ...
            if (renderStack != null) RenderItemAtCoords(renderStack, d + 0.5f, d1 + 0.85f, d2 + 0.5f, f);
        }
        ...
        if (te.shouldRenderRotateOffset()) {
            ...
            RenderRotatedModelGroup(
                "Ring03",
                te.getInnerRingRotation()
                    .copy()
                    .sub(
                        te.getInnerRingRotationSpeed()
                            .copy()
                            .scale(te.getRotateOffset())));
            RenderRotatedModelGroup(
                "Ring01",
                te.getMiddleRingRotation()
                    .copy()
                    .sub(
                        te.getMiddleRingRotationSpeed()
                            .copy()
                            .scale(te.getRotateOffset())));
            RenderRotatedModelGroup(
                "Ring02",
                te.getOuterRingRotation()
                    .copy()
                    .sub(
                        te.getOuterRingRotationSpeed()
                            .copy()
                            .scale(te.getRotateOffset())));
    ...
    private void RenderItemAtCoords(ItemStack item, double x, double y, double z, float partialTick) {
        ...
        renderItem
            .doRender(AMGuiHelper.instance.dummyItem, x, y, z, AMGuiHelper.instance.dummyItem.rotationYaw, partialTick);
    }
}
