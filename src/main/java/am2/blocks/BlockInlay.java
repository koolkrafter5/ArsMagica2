...
public class BlockInlay extends BlockRailBase {
    ...
    public void registerBlockIcons(IIconRegister IIconRegister) {
        String[] textures = new String[] { "Inlay_Straight_Redstone", "Inlay_Straight_Iron", "Inlay_Straight_Gold",
            "Inlay_Corner_Redstone", "Inlay_Corner_Iron", "Inlay_Corner_Gold" };
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        if (world.isRemote && world.getBlock(x, y - 1, z)
            ...
            AMParticle particle = (AMParticle) AMCore.proxy.particleManager.spawn(
                world,
                AMCore.config.FullGFX() ? "radiant" : "sparkle2",
                x + rand.nextFloat(),
                y,
                z + rand.nextFloat());
            if (particle != null) {
                ...
                if (this == BlocksCommonProxy.redstoneInlay) particle.setRGBColorF(1.0f, 0.4f, 0.4f);
                else if (this == BlocksCommonProxy.ironInlay) particle.setRGBColorF(1.0f, 1.0f, 1.0f);
                else if (this == BlocksCommonProxy.goldInlay) particle.setRGBColorF(1.0f, 1.0f, 0.2f);
            }
        }
    }
    ...
    public float getRailMaxSpeed(World world, EntityMinecart cart, int y, int x, int z) {
        if (this.material == TYPE_REDSTONE) return 0.7f;
        return 0.4f;
    }
            ...
            for (int i = -1; i <= 1; ++i) {
                for (int j = -1; j <= 1; ++j) {
                    if (world.getBlock(x + i, y, z + j) == this
                        && (world.getBlockMetadata(x + i, y, z + j) & 0x8) != 0) {
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        if (!world.isRemote) {
            int meta = world.getBlockMetadata(x, y, z);
            if (meta == 6) checkPattern(world, x, y, z);
            world.scheduleBlockUpdate(x, y, z, this, tickRate(world));
        }
    }
    ...
    private void checkPattern(World world, int x, int y, int z) {
        ...
        if (allGood) {
            if (!world.isRemote) {
                if (!checkForIceEffigy(world, x, y, z)) checkForLightningEffigy(world, x, y, z);
            }
        }
    }

    private boolean checkForIceEffigy(World world, int x, int y, int z) {
        if (world.getBlock(x + 1, y, z + 1) == BlocksCommonProxy.AMOres
            && world.getBlockMetadata(x + 1, y, z + 1) == BlockAMOre.META_BLUE_TOPAZ_BLOCK) {
            if (world.getBlock(x + 1, y + 1, z + 1) == BlocksCommonProxy.AMOres
                && world.getBlockMetadata(x + 1, y + 1, z + 1) == BlockAMOre.META_BLUE_TOPAZ_BLOCK) {
                if (world.getBlock(x + 1, y + 2, z + 1) == Blocks.ice) {
                    int iceMeta = world.getBlockMetadata(x + 1, y + 2, z + 1);
                    if (iceMeta > 0) {
                        AMCore.proxy.particleManager
                            .RibbonFromPointToPoint(world, x + 1.5, y + 2, z + 1.5, x + 2, y + 3, z + 2);
                    }
                    if (iceMeta >= 3) {
                        BossSpawnHelper.instance.onIceEffigyBuilt(world, x + 1, y, z + 1);
                    } else {
                        List<EntitySnowman> snowmen = world.getEntitiesWithinAABB(
                            EntitySnowman.class,
                            AxisAlignedBB.getBoundingBox(x - 9, y - 2, z - 9, x + 10, y + 4, z + 10));
                        if (snowmen.size() > 0) {
                            snowmen.get(0)
                                .attackEntityFrom(DamageSources.unsummon, 5000);
                            iceMeta++;
                            world.setBlockMetadataWithNotify(x + 1, y + 2, z + 1, iceMeta, 2);
                            AMCore.proxy.particleManager
                                .BeamFromEntityToPoint(world, snowmen.get(0), x + 1.5, y + 2.5, z + 1.5);
                        }
                    }
                    return true;
                }
        ...
        if (world.getBlock(x + 1, y, z + 1) == BlocksCommonProxy.manaBattery) {
            if (world.getBlock(x + 1, y + 1, z + 1) == Blocks.iron_bars) {
                if (world.getBlock(x + 1, y + 2, z + 1) == Blocks.iron_bars) {
                    ...
                    if (!world.isRemote) {
                        for (int i = 0; i < 5; ++i) AMCore.proxy.particleManager
                            .BoltFromPointToPoint(world, x + 1.5, y + 30, z + 1.5, x + 1.5, y + 2, z + 1.5);
                        for (int i = 0; i < fenceMeta; ++i) world
                            .playSoundEffect(x, y, z, "ambient.weather.thunder", 1.0f, world.rand.nextFloat() + 0.5f);
                    }
                    if (fenceMeta >= 3) {
    ...
    public void onMinecartPass(World world, EntityMinecart cart, int x, int y, int z) {
        ...
        long millisSinceLastTeleport = cart.getEntityData()
            .getLong(minecartGoldInlayKey);

        int meta = world.getBlockMetadata(x, y, z);
        if (this.material == TYPE_REDSTONE) {
            float limit = 2f;
            if (meta == 1) {
                if (cart.motionX > 0 && cart.motionX < limit) cart.motionX *= 1.1f;
                else if (cart.motionX < 0 && cart.motionX > -limit) cart.motionX *= 1.1f;
            } else if (meta == 0) {
                if (cart.motionZ > 0 && cart.motionZ < limit) cart.motionZ *= 1.1f;
                else if (cart.motionZ < 0 && cart.motionZ > -limit) cart.motionZ *= 1.1f;
            }
        } else if (this.material == TYPE_IRON) {
            if (meta == 1) {
                cart.motionX = -cart.motionX * 0.5f;
                if (cart.motionX < 0) cart.setPosition(x - 0.02, cart.posY, cart.posZ);
                else if (cart.motionX > 0) cart.setPosition(x + 1.02, cart.posY, cart.posZ);
            } else if (meta == 0) {
                cart.motionZ = -cart.motionZ * 0.5f;
                if (cart.motionZ < 0) cart.setPosition(cart.posX, cart.posY, z - 0.02);
                else if (cart.motionZ > 0) cart.setPosition(cart.posX, cart.posY, z + 1.02);
            }
            ...
            int teleportMeta = teleportLocation != null
                ? world.getBlockMetadata((int) teleportLocation.x, (int) teleportLocation.x, (int) teleportLocation.z)
                : -1;
            ...
            if (teleportLocation != null && (teleportMeta == 1 || teleportMeta == 0) && cooldownPassed) {
                ...
                cart.getEntityData()
                    .setLong(minecartGoldInlayKey, time);
            }
        }
    }
}
