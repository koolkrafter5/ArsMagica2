        ...
        if (!world.isRemote) {
            ...
            for (EntityPlayer p : playersInRange) {
                if (!p.isPotionActive(Potion.regeneration))
                    p.addPotionEffect(new PotionEffect(Potion.regeneration.id, 110, 1));
            }
        }
    }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random par5Random) {
        ...
        for (int i = 0; i < 360; i += increment) {
            ...
            AMParticle effect = (AMParticle) AMCore.instance.proxy.particleManager
                .spawn(world, "sparkle", x + 0.5, posY, z + 0.5);
            if (effect != null) {
                ...
                effect.AddParticleController(
                    new ParticleExpandingCollapsingRingAtPoint(effect, posX, posY, posZ, 0.3, 3, 0.2, 1, false)
                        .setExpanding());
                // effect.AddParticleController(new ParticleOrbitPoint(effect, x+0.5, y, z+0.5, 1,
                // false).SetTargetDistance(0.35f + par5Random.nextFloat() *
                ...
                effect.AddParticleController(
                    new ParticleFadeOut(effect, 2, false).setFadeSpeed(0.05f)
