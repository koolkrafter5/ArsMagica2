...
public class BlockSpellSealedDoor extends BlockDoor implements ITileEntityProvider {
    ...
    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int direction, float xOffset,
        float yOffset, float zOffset) {

        if (world.getBlock(x, y - 1, z) == BlocksCommonProxy.spellSealedDoor) y--;
        ...
        if (!world.isRemote) {
            ...
            if (KeystoneUtilities.instance.canPlayerAccess((IKeystoneLockable) te, player, KeystoneAccessType.USE)) {
                if (player.isSneaking()) {
                    FMLNetworkHandler
                        .openGui(player, AMCore.instance, ArsMagicaGuiIdList.GUI_SPELL_SEALED_DOOR, world, x, y, z);
                } else {
                    return false;
                }
            }
    ...
    public void breakBlock(World world, int i, int j, int k, Block par5, int metadata) {
        ...
        if (world.getBlock(i, j - 1, k) == BlocksCommonProxy.spellSealedDoor) j--;
    ...
    public boolean removedByPlayer(World world, EntityPlayer player, int x, int y, int z) {
        ...
        if (world.getBlock(x, y - 1, z) == BlocksCommonProxy.keystoneDoor) y--;
    ...
    public void onBlockHarvested(World world, int x, int y, int z, int meta, EntityPlayer player) {
        ...
        if (!KeystoneUtilities.instance.canPlayerAccess(lockable, player, KeystoneAccessType.BREAK)) return;
    ...
    public boolean applyComponentToDoor(IBlockAccess access, ISpellComponent component, int x, int y, int z) {
        ...
        if (access.getBlock(x, y - 1, z) == BlocksCommonProxy.spellSealedDoor) y--;
