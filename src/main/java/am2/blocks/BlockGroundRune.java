...
public abstract class BlockGroundRune extends AMBlockContainer {
    ...
    public boolean canPlaceBlockAt(World world, int i, int j, int k) {
        return world.getBlock(i, j - 1, k)
            .isNormalCube(world, i, j - 1, k) && world.isAirBlock(i, j, k);
    }
        ...
        if (list.size() > 0) {
            if (ActivateRune(world, list, i, j, k)) {
                ...
                if (!isPermanent(world, i, j, k, meta)) {
                    ...
                    if (--numTriggers <= 0) world.setBlock(i, j, k, Blocks.air);
                    else setNumTriggers(world, i, j, k, meta, numTriggers);
                }
