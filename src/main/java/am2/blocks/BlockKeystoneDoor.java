...
public class BlockKeystoneDoor extends BlockDoor implements ITileEntityProvider {
        ...
        if (!world.isRemote) {
            ...
            if (KeystoneUtilities.instance.canPlayerAccess((IKeystoneLockable) te, player, KeystoneAccessType.USE)) {
                if (player.isSneaking()) {
                    FMLNetworkHandler
                        .openGui(player, AMCore.instance, ArsMagicaGuiIdList.GUI_KEYSTONE_LOCKABLE, world, x, y, z);
                } else {
                    ...
                    CompendiumUnlockHandler.unlockEntry(
    ...
    private void activateNeighbors(World world, int x, int y, int z, EntityPlayer player, int direction, float xOffset,
        float yOffset, float zOffset) {
