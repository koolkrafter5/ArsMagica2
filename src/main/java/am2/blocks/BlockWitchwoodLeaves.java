...
public class BlockWitchwoodLeaves extends BlockLeaves {
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random) {
        ...
        if (!AMCore.config.witchwoodLeafPFX()) return;
        ...
        if (par5Random.nextInt(300) == 0 && par1World.isAirBlock(par2, par3 - 1, par4)) {
            AMParticle particle = (AMParticle) AMCore.proxy.particleManager.spawn(
                par1World,
                "leaf",
                par2 + par5Random.nextDouble(),
                par3 + par5Random.nextDouble(),
                par4 + par5Random.nextDouble());
            if (particle != null) {
                ...
                particle.AddParticleController(
                    new ParticlePendulum(particle, 0.2f, 0.15f + par5Random.nextFloat() * 0.2f, 2, false));
            }
        }
    }
    ...
    public void updateTick(World world, int xx, int yy, int zz, Random p_149674_5_) { // rework inefficient and
                                                                                      // needlessly complex minecraft
                                                                                      // algorithm for detecting leaves
                                                                                      // to drop
        if (!world.isRemote) {
                ...
                for (int y = -7; y <= 7; y++) {
                    for (int z = -7; z <= 7; z++) {
                        if (world.getBlock(x + xx, y + yy, z + zz)
                            .canSustainLeaves(world, xx, yy, zz)) {
                            toRemove = false;
                        }
                    }
                }
