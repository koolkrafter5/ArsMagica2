...
public class AMPacketProcessorClient extends AMPacketProcessorServer {
            ...
            switch (packetID) {
                ...
                case AMPacketIDs.PARTICLE_SPAWN_SPECIAL:
                    AMCore.instance.proxy.particleManager
                        .handleClientPacketData(Minecraft.getMinecraft().theWorld, remaining);
                    break;
    ...
    private void handleRcvPowerPaths(byte[] data) {
        ...
        if (bite == 0) {
            ...
            if (te != null && te instanceof IPowerNode) PowerNodeRegistry.For(Minecraft.getMinecraft().theWorld)
    ...
    private void handleSpellApplyEffect(byte[] data) {
        ...
        if (caster == null || target == null
            ...
            || !(target instanceof EntityLivingBase)) return;
    ...
    private void handleSyncBetaParticles(byte[] data) {
        EntityPlayer localPlayer = Minecraft.getMinecraft().thePlayer;
        if (!AMCore.proxy.playerTracker.hasAA(localPlayer)) return;
    ...
    private void handleSpellCast(byte[] data) {
        ...
        if (caster == null || !(caster instanceof EntityLivingBase)) return;
