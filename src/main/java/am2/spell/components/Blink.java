...
public class Blink implements ISpellComponent, IRitualInteraction {
    ...
    public boolean applyEffectBlock(ItemStack stack, World world, int blockx, int blocky, int blockz, int blockFace,
        ...
        if (AMCore.config.enableSpatialVortex() && world.getTileEntity(blockx, blocky, blockz) != null
            ...
            if (world.getTileEntity(blockx, blocky, blockz) instanceof TileEntityBlackAurem) {
                if (RitualShapeHelper.instance.checkForRitual(this, world, blockx, blocky, blockz) != null) {
                    ...
                    ExtendedProperties.For((EntityPlayer) caster)
                        .addToExtraVariables(
                            "spatialvortex_" + blockx + "_" + blocky + "_" + blockz + "_" + world.provider.dimensionId,
                            "AN"); // AN: Active Normal. Other modes of operation not yet present.
                }
            }
        }
        return false;
    ...
    public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
        ...
        double motionX = -MathHelper.sin((target.rotationYaw / 180F) * 3.141593F)
            * MathHelper.cos((target.rotationPitch / 180F) * 3.141593F)
            * distance;
        ...
        while (!coordsValid && distance > 0) {
            ...
            TileEntityAstralBarrier blocker = DimensionUtilities
                .GetBlockingAstralBarrier(world, (int) newX, (int) newY, (int) newZ, keystoneKeys);
            while (blocker != null) {
        ...
        if (!world.isRemote) {
            if (!coordsValid && target instanceof EntityPlayer) {
                ((EntityPlayer) target)
                    .addChatMessage(new ChatComponentText("Can't find a place to blink forward to."));
                return false;
            }
        }
                ...
                particle.AddParticleController(
                    new ParticleMoveOnHeading(
                        ...
                        MathHelper.wrapAngleTo180_double(
                            (target instanceof EntityLivingBase ? ((EntityLivingBase) target).rotationYawHead
                                : target.rotationYaw) + 90),
                        MathHelper.wrapAngleTo180_double(target.rotationPitch),
    ...
    protected double GetTeleportDistance(ItemStack stack, EntityLivingBase caster, Entity target) {
        double distance = 12;
        return SpellUtils.instance
            .getModifiedDouble_Add(distance, stack, caster, target, caster.worldObj, 0, SpellModifiers.RANGE);
    }
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemsCommonProxy.rune.META_PURPLE),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_COSMICDUST), Items.ender_pearl };
    }
    ...
    public ItemStack[] getReagents() {
        return new ItemStack[] { new ItemStack(Items.ender_pearl),
