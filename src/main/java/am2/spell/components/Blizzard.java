...
public class Blizzard implements ISpellComponent {
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.essence, 1, ItemsCommonProxy.essence.META_ICE),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_BLUETOPAZ),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_MOONSTONEFRAGMENT), Blocks.ice,
    ...
    private boolean spawnBlizzard(ItemStack stack, World world, EntityLivingBase caster, Entity target, double x,
        ...
        List<EntitySpellEffect> zones = world.getEntitiesWithinAABB(
            EntitySpellEffect.class,
            AxisAlignedBB.getBoundingBox(x - 10, y - 10, z - 10, x + 10, y + 10, z + 10));

        for (EntitySpellEffect zone : zones) {
            if (zone.isBlizzard()) return false;
        }

        if (!world.isRemote) {
            int radius = SpellUtils.instance
                .getModifiedInt_Add(2, stack, caster, target, world, 0, SpellModifiers.RADIUS);
            double damage = SpellUtils.instance
                .getModifiedDouble_Mul(1, stack, caster, target, world, 0, SpellModifiers.DAMAGE);
