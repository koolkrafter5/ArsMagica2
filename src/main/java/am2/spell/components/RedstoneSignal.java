...
public class RedstoneSignal implements ISpellComponent {
    ...
    public boolean applyEffectBlock(ItemStack stack, World world, int blockx, int blocky, int blockz, int blockFace,
        ...
        if (block == Blocks.air || block.getMaterial()
            .isLiquid() || block instanceof BlockDoor || block instanceof BlockTrapDoor) {
            return false;
        }

        int activeTime = SpellUtils.instance
            .getModifiedInt_Mul(10, stack, caster, caster, world, 0, SpellModifiers.DURATION);
        activeTime = SpellUtils.instance.modifyDurationBasedOnArmor(caster, activeTime);
        MysteriumPatchesFixesMagicka.providingRedstone
            .add(new int[] { world.provider.dimensionId, blockx, blocky, blockz, activeTime });
    ...
    public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target,
        ...
        for (int i = 0; i < 3; i++) world.spawnParticle(
            "reddust",
            x,
            y,
            z,
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemsCommonProxy.rune.META_RED), Items.redstone,
