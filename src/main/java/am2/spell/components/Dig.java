...
public class Dig implements ISpellComponent {
    ...
    public boolean applyEffectBlock(ItemStack stack, World world, int blockx, int blocky, int blockz, int blockFace,
        ...
        if (te != null) {
            if (!AMCore.config.getDigBreaksTileEntities()) return false;

            if (te instanceof IKeystoneLockable && !KeystoneUtilities.instance.canPlayerAccess(
                (IKeystoneLockable) te,
                DummyEntityPlayer.fromEntityLiving(caster),
                KeystoneAccessType.BREAK)) return false;
        }
        ...
        if (ForgeEventFactory.doPlayerHarvestCheck(casterPlayer, block, true)) {
            ...
            if (!world.isRemote) {
                BreakEvent event = ForgeHooks.onBlockBreakEvent(
                    world,
                    ((EntityPlayerMP) casterPlayer).theItemInWorldManager.getGameType(),
                    (EntityPlayerMP) casterPlayer,
                    blockx,
                    blocky,
                    blockz);
                if (event.isCanceled()) {
