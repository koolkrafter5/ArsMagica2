...
public class DivineIntervention implements ISpellComponent {
    ...
    public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
        ...
        if (((EntityLivingBase) target).isPotionActive(BuffList.astralDistortion.id)) {
            if (target instanceof EntityPlayer) ((EntityPlayer) target)
                .addChatMessage(new ChatComponentText("The distortion around you prevents you from teleporting"));
            return true;
        }
        ...
        if (target.dimension == 1) {
            ...
            ChunkCoordinates coords = target instanceof EntityPlayer
                ? ((EntityPlayer) target).getBedLocation(target.dimension)
                : null;
            if (coords == null || (coords.posX == 0 && coords.posY == 0 && coords.posZ == 0)) {
                coords = world.getSpawnPoint();
            }
            int yPos = coords.posY;
            while (world.getBlock(coords.posX, yPos, coords.posZ) != Blocks.air
                && world.getBlock(coords.posX, yPos + 1, coords.posZ) != Blocks.air) {
                yPos++;
            }
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemsCommonProxy.rune.META_PURPLE),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_CELESTIALFISH), Items.bed,
            Items.ender_pearl };
    }
