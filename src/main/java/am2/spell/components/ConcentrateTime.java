...
public class ConcentrateTime implements ISpellComponent {
    ...
    public boolean applyEffectBlock(ItemStack stack, World world, int blockx, int blocky, int blockz, int blockFace,
        ...
        if (block == Blocks.bedrock) { // global. The disable switch is in the method being called
            ...
            if (SpellUtils.instance.casterHasMana(caster, 200000)) { // above normal max. mana, but there are multiple
                                                                     // possible ways to achieve it.
                ep.deductMana(200000);
                MinecraftServer server = MinecraftServer.getServer();
                if ((server != null) && (server.getConfigurationManager() != null)) { // server side, or client side
                                                                                      // singleplayer
                    MysteriumPatchesFixesMagicka.changeTickrate(
                        (int) (20
                            * ((SpellUtils.instance.countModifiers(SpellModifiers.BUFF_POWER, stack, 0) + 1) * 1.5)));
                    int duration = (int) (SpellUtils.instance.getModifiedInt_Mul(
                        ...
                        world,
                        0,
                        SpellModifiers.DURATION) / 2);
                    duration = SpellUtils.instance.modifyDurationBasedOnArmor(caster, duration);
                    MysteriumPatchesFixesMagicka.countdownToChangeBack = duration; // if the server is reset, this will
                                                                                   // reset anyway, so there's no issues
                                                                                   // with not storing this in nbt
        ...
        ExtendedProperties ep = ExtendedProperties.For(caster);
        if (ep != null) {
            ep.addToExtraVariables(
                "accelerated_fast_tile_" + blockx
                    + "_"
                    + blocky
                    + "_"
                    + blockz
                    ...
                    + "_"
                    + (SpellUtils.instance.countModifiers(SpellModifiers.BUFF_POWER, stack, 0) + 2),
    ...
    @Override
    public boolean applyEffectEntity(ItemStack stack, World world, net.minecraft.entity.EntityLivingBase caster,
        Entity target) {
        ...
        if (ep != null) {
            ep.addToExtraVariables(
                "accelerated_fast_entity_"
                    + (SpellUtils.instance.countModifiers(SpellModifiers.BUFF_POWER, stack, 0) + 2)
                    + "_"
                    + targetBase.getUniqueID()
                        .toString(),
                String.valueOf(duration));
        }
        return true;
    ...
    public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target,
        ...
        if (particle != null) {
            particle.AddParticleController(
                new ParticleOrbitEntity(particle, caster, 0.1f, 1, false).SetTargetDistance(rand.nextDouble() + 0.5));
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemsCommonProxy.rune.META_YELLOW),
            ...
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_SUNSTONEFRAGMENT), Items.clock, };
    }
