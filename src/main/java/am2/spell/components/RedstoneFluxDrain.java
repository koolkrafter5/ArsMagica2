...
public class RedstoneFluxDrain implements ISpellComponent {
    ...
    public boolean applyEffectBlock(ItemStack stack, World world, int blockx, int blocky, int blockz, int blockFace,
        ...
        if (te != null && te instanceof IEnergyHandler) {
            int rfToSteal = SpellUtils.instance
                .getModifiedInt_Add(1000, stack, caster, caster, world, 0, SpellModifiers.DAMAGE);
    ...
    private void giveBackEnergy(EntityPlayer caster, int rfToGiveBack) {
        ...
        for (int i = 0; i < cPlayer.inventory.mainInventory.length; i++) {
            ...
            if (cPlayer.inventory.mainInventory[i] != null
                && cPlayer.inventory.mainInventory[i].getItem() instanceof IEnergyContainerItem) {
                int inserted = ((IEnergyContainerItem) cPlayer.inventory.mainInventory[i].getItem())
                    .receiveEnergy(cPlayer.inventory.mainInventory[i], rfToGiveBack, false);
        ...
        for (int i = 0; i < cPlayer.inventory.armorInventory.length; i++) {
            ...
            if (cPlayer.inventory.armorInventory[i] != null
                && cPlayer.inventory.armorInventory[i].getItem() instanceof IEnergyContainerItem) {
                int inserted = ((IEnergyContainerItem) cPlayer.inventory.armorInventory[i].getItem())
                    .receiveEnergy(cPlayer.inventory.armorInventory[i], rfToGiveBack, false);
                rfToGiveBack -= inserted;
            }
        }
        return;
    ...
    public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
        ...
        int rfToSteal = SpellUtils.instance
            .getModifiedInt_Add(1000, stack, caster, target, world, 0, SpellModifiers.DAMAGE);
        int rfToGiveBack = SpellUtils.instance
            .getModifiedInt_Add(1000, stack, caster, target, world, 0, SpellModifiers.DAMAGE);
        ...
        for (int i = 0; i < player.inventory.mainInventory.length; i++) {
            ...
            if (player.inventory.mainInventory[i] != null
                && player.inventory.mainInventory[i].getItem() instanceof IEnergyContainerItem) {
                int extracted = ((IEnergyContainerItem) player.inventory.mainInventory[i].getItem())
                    .extractEnergy(player.inventory.mainInventory[i], rfToSteal, false);
                rfToSteal -= extracted;
            }
        }
        for (int i = 0; i < player.inventory.armorInventory.length; i++) {
            ...
            if (player.inventory.armorInventory[i] != null
                ...
                int extracted = ((IEnergyContainerItem) player.inventory.armorInventory[i].getItem())
                    .extractEnergy(player.inventory.armorInventory[i], rfToSteal, false);
                rfToSteal -= extracted;
            }
        }
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemsCommonProxy.rune.META_CYAN),
            new ItemStack(Items.water_bucket), new ItemStack(Items.diamond),
