...
public class SpellRecipeManager {
    ...
    public void RegisterRecipe(ISpellPart part) {
        ...
        if (recipe == null) {
            LogHelper.info(
                "Component %s has been registered with no craftable recipe - is this intentional?  If so, return a 0-length array for recipe.",
                SkillManager.instance.getDisplayName(part));
            return;
        }
        if (recipe.length == 0) {
            return;
        ...
        for (int i = 0; i < recipe.length; ++i) {
            ...
            if (o instanceof Item) {
                ...
                if (((String) o).toLowerCase()
                    .startsWith("e:")) {
                    if (i == recipe.length - 1 || !(recipe[i + 1] instanceof Integer)) {
                        LogHelper.warn(
                            "Error registering recipe.  Power must be declared in Integer pairs (type flags, quantity)).");
                        return;
                    }
    ...
    private ISpellPart matchRecipe(ArrayList<ItemStack> recipe) {
        ...
        for (ArrayList<Object> arr : safeCopy.keySet()) {
            if (arr.size() != recipe.size()) toRemove.add(arr);
        }
        ...
        if (safeCopy.size() > 0) {
            ISpellPart part = safeCopy.values()
                .iterator()
                .next();
            LogHelper.info(
                "Matched Spell Component: %s",
                part.getClass()
                    .toString());
            return part;
        }
    ...
    private boolean compareItemStacks(ItemStack target, ItemStack input) {
        if (target.getItem() == ItemsCommonProxy.essence
            && target.getItemDamage() > ItemsCommonProxy.essence.META_MAX) {
