...
public class BuffStatModifiers {
    ...
    public void applyStatModifiersBasedOnBuffs(EntityLivingBase entity) {
        // entangled
        applyOrRemoveModifiersForBuff(
            entity,
            BuffList.entangled.id,
            new KeyValuePair(SharedMonsterAttributes.movementSpeed, entangled));
        // frost slow
        applyOrRemoveScalingModifiersForBuff(
            entity,
            BuffList.frostSlowed.id,
            SharedMonsterAttributes.movementSpeed,
            frostSlow_Diminished,
            frostSlow_Normal,
            frostSlow_Augmented);
        // fury
        applyOrRemoveModifiersForBuff(
            entity,
            BuffList.fury.id,
            new KeyValuePair(SharedMonsterAttributes.movementSpeed, furyMoveMod),
            new KeyValuePair(SharedMonsterAttributes.attackDamage, furyDmgMod));
        // haste
        applyOrRemoveScalingModifiersForBuff(
            entity,
            BuffList.haste.id,
            SharedMonsterAttributes.movementSpeed,
            hasteSpeedBoost_Diminished,
            hasteSpeedBoost_Normal,
            hasteSpeedBoost_Augmented);
    }

    private void applyOrRemoveModifiersForBuff(EntityLivingBase entity, int buffID,
        KeyValuePair<IAttribute, AttributeModifier>... modifiers) {
    ...
    private void applyOrRemoveScalingModifiersForBuff(EntityLivingBase entity, int potionID, IAttribute attribute,
        ...
        if (entity.isPotionActive(potionID)) {
            int magnitude = entity.getActivePotionEffect(Potion.potionTypes[potionID])
                .getAmplifier();
    ...
    private void applyAllModifiers(EntityLivingBase entity, KeyValuePair<IAttribute, AttributeModifier>... modifiers) {
        for (KeyValuePair<IAttribute, AttributeModifier> entry : modifiers) {
            ...
            AttributeModifier currentModifier = inst.getModifier(
                entry.getValue()
                    .getID());
    ...
    private static final AttributeModifier hasteSpeedBoost_Diminished = (new AttributeModifier(
        hasteID,
        "Haste Speed Boost (Diminished)",
        0.3D,
        2));
    private static final AttributeModifier hasteSpeedBoost_Normal = (new AttributeModifier(
        hasteID,
        "Haste Speed Boost (Normal)",
        0.6D,
        2));
    private static final AttributeModifier hasteSpeedBoost_Augmented = (new AttributeModifier(
        hasteID,
        "Haste Speed Boost (Augmented)",
        1.2D,
        2));

    // * Frost Slow *//
    private static final UUID frostSlowID = UUID.fromString("03B0A79B-9569-43AE-BFE3-820D993D4A64");
    private static final AttributeModifier frostSlow_Diminished = (new AttributeModifier(
        frostSlowID,
        "Frost Slow (Diminished)",
        -0.2,
        2));
    private static final AttributeModifier frostSlow_Normal = (new AttributeModifier(
        frostSlowID,
        "Frost Slow (Normal)",
        -0.5,
        2));
    private static final AttributeModifier frostSlow_Augmented = (new AttributeModifier(
        frostSlowID,
        "Frost Slow (Augmented)",
        -0.8,
        2));
}
