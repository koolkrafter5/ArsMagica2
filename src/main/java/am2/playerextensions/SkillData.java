...
public class SkillData implements IExtendedEntityProperties, ISkillData {
    ...
    public void learn(ISkillTreeEntry entry) {
        if (entity.worldObj.isRemote) {
            ...
            int id = entry.getID();
            if (entry instanceof ISpellComponent) id += SkillManager.instance.COMPONENT_OFFSET;
            else if (entry instanceof ISpellModifier) id += SkillManager.instance.MODIFIER_OFFSET;
            else if (!(entry instanceof ISpellShape)) id += SkillManager.instance.TALENT_OFFSET;
            writer.add(id);
        ...
        try {
            ...
            switch (subID) {
                case SPELL_LEARNED:
                    ...
                    int type = rdr.getInt();
                    learn(id, type);
                    if (!entity.worldObj.isRemote) forceSync();
                    break;
                case CLIENT_SYNC:
    ...
    public void setFullSync() {
        this.ticksToSync = 0;
        this.updateFlags = this.updateFlags | KNOWN_SHAPE_UPDATE
            | KNOWN_COMPONENT_UPDATE
            | KNOWN_MODIFIER_UPDATE
            | KNOWN_TALENTS_UPDATE
            | NUM_SPELL_POINTS;
    ...
    public void handleExtendedPropertySync() {
        ...
        if (!entity.worldObj.isRemote && this.hasUpdate()) {
            ...
            AMNetHandler.INSTANCE.sendPacketToAllClientsNear(
                ...
                entity.posZ,
                32,
                AMPacketIDs.SYNC_SPELL_KNOWLEDGE,
                data);
        }
    }
    ...
    public LearnStates getLearnState(SkillTreeEntry entry, EntityPlayer player) {
        ...
        LearnStates state = entry.enabled
            ? (sk.getSpellPoints(pointType) > 0 ? LearnStates.CAN_LEARN : LearnStates.CANNOT_LEARN)
            : LearnStates.DISABLED;
        ...
        if (sk.getPrimaryTree() != SkillTrees.None && entry.tree != SkillTrees.Talents
            && sk.getPrimaryTree() != entry.tree
            && entry.tier >= AMCore.config.getSkillTreeSecondaryTierCap()) {
    ...
    public void respec() {
        ...
        for (Integer i : shapesKnown) {
            SkillPointTypes type = SkillTreeManager.instance
                .getSkillPointTypeForPart(SkillManager.instance.getSkill(i));
