...
public class EntityItemWatcher {
    ...
    public void tick() {
        ...
        for (EntityItem item : watchedItems) {
            ...
            if (!item.isBurning()
                && (Math.abs(item.motionX) > 0.01 || Math.abs(item.motionY) > 0.01 || Math.abs(item.motionZ) > 0.01))
                continue;
    ...
    public void registerInlayBlock(Block inlayBlock) {
        if (!this.inlayBlocks.contains(inlayBlock)) this.inlayBlocks.add(inlayBlock);
    }

    public void registerWatchableItem(Item item) {
        if (!this.itemsToWatch.contains(item)) this.itemsToWatch.add(item);
    }
