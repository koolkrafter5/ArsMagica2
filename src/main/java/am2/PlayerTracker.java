...
public class PlayerTracker {
    ...
    public static void storeExtendedPropertiesForRespawn(EntityPlayer player) {
        // extended properties
        // ================================================================================
        if (storedExtProps_death.containsKey(player.getUniqueID())) storedExtProps_death.remove(player.getUniqueID());
        ...
        // rift storage
        // ================================================================================
        if (riftStorage_death.containsKey(player.getUniqueID())) riftStorage_death.remove(player.getUniqueID());
        NBTTagCompound saveRift = new NBTTagCompound();
        RiftStorage.For(player)
            .saveNBTData(saveRift);
        ...
        // affinity storage
        // ================================================================================
        if (affinityStorage_death.containsKey(player.getUniqueID())) affinityStorage_death.remove(player.getUniqueID());
        NBTTagCompound saveAffinity = new NBTTagCompound();
        AffinityData.For(player)
            .saveNBTData(saveAffinity);
        ...
        SkillData.For(player)
            .saveNBTData(saveSpellKnowledge);
    ...
    public static void storeSoulboundItemsForRespawn(EntityPlayer player) {
        if (soulbound_Storage.containsKey(player.getUniqueID())) soulbound_Storage.remove(player.getUniqueID());
    ...
    public static void storeSoulboundItemForRespawn(EntityPlayer player, ItemStack stack) {
        if (!soulbound_Storage.containsKey(player.getUniqueID())) return;
        ...
        while (soulboundItems.containsKey(slotTest)) {
            slotTest++;
            if (slotTest == player.inventory.mainInventory.length) slotTest += player.inventory.armorInventory.length;
        }
    ...
    public int getAAL(EntityPlayer thePlayer) {
        ...
        return thePlayer.getDisplayName()
            .equalsIgnoreCase("Nlghtwing") ? 5 : 0;
    }
}
